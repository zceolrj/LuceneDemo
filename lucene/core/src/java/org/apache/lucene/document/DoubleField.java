...
/**
 * <p>
 * Field that indexes <code>double</code> values for efficient range filtering
 * and sorting. Here's an example usage:
 * 
 ...
 *
 * <p>
 * To perform range querying or filtering against a <code>DoubleField</code>,
 * use {@link NumericRangeQuery} or {@link NumericRangeFilter}. To sort
 * according to a <code>DoubleField</code>, use the normal numeric sort types,
 * eg {@link org.apache.lucene.search.SortField.Type#DOUBLE}.
 * <code>DoubleField</code> values can also be loaded directly from
 * {@link FieldCache}.
 * </p>
 *
 * <p>
 * You may add the same field name as an <code>DoubleField</code> to the same
 ...
 * sort, you should separately index a single-valued <code>DoubleField</code>.
 * </p>
 *
 * <p>
 * A <code>DoubleField</code> will consume somewhat more disk space in the index
 * than an ordinary single-valued field. However, for a typical index that
 ...
 * disk space consumption versus performance. You can create a custom
 * {@link FieldType} and invoke the {@link FieldType#setNumericPrecisionStep}
 * method if you'd like to change the value. Note that you must also specify a
 * congruent value when creating {@link NumericRangeQuery} or
 * {@link NumericRangeFilter}. For low cardinality fields larger precision steps
 * are good. If the cardinality is &lt; 100, it is fair to use
 * {@link Integer#MAX_VALUE}, which produces one term per value.
 *
 * <p>
...
public final class DoubleField extends Field
{
	...
	/**
	 * Type for a DoubleField that is not stored: normalization factors,
	 * frequencies, and positions are omitted.
	 */
	...
	/**
	 * Type for a stored DoubleField: normalization factors, frequencies, and
	...
	/**
	 * Creates a stored or un-stored DoubleField with the provided value and
	 * default <code>precisionStep</code>
	 ...
	 * @param name field name
	 * @param value 64-bit double value
	 * @param type customized field type: must have
	 *            {@link FieldType#numericType()} of
	 *            {@link FieldType.NumericType#DOUBLE}.
	 * @throws IllegalArgumentException if the field name or type is null, or if
	 *             the field type does not have a DOUBLE numericType()
	 */
	public DoubleField(String name, double value, FieldType type)
	{
		...
		if (type.numericType() != FieldType.NumericType.DOUBLE)
		{
			throw new IllegalArgumentException(
					"type.numericType() must be DOUBLE but got "
							+ type.numericType());
		}
		fieldsData = Double.valueOf(value);
	}
