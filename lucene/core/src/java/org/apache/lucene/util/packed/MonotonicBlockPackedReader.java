...
public final class MonotonicBlockPackedReader
{
	...
	/** Sole constructor. */
	public MonotonicBlockPackedReader(IndexInput in, int packedIntsVersion,
			int blockSize, long valueCount, boolean direct) throws IOException
	{
		...
		for (int i = 0; i < numBlocks; ++i)
		{
			...
			{
				final int size = (int) Math.min(blockSize, valueCount
						- (long) i * blockSize);
				if (direct)
				{
					final long pointer = in.getFilePointer();
					subReaders[i] = PackedInts.getDirectReaderNoHeader(in,
							PackedInts.Format.PACKED, packedIntsVersion, size,
							bitsPerValue);
					in.seek(pointer
				...
				{
					subReaders[i] = PackedInts.getReaderNoHeader(in,
							PackedInts.Format.PACKED, packedIntsVersion, size,
							bitsPerValue);
				}
			}
	...
	public long get(long index)
	{
		...
		return minValues[block] + (long) (idx * averages[block])
				+ zigZagDecode(subReaders[block].get(idx));
	}
