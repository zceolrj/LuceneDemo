...
abstract class AbstractBlockPackedWriter
{
	 ...
	 * Sole constructor.
	 * 
	 * @param blockSize the number of values of a single block, must be a
	...
	{
		final PackedInts.Encoder encoder = PackedInts.getEncoder(
				PackedInts.Format.PACKED, PackedInts.VERSION_CURRENT,
				bitsRequired);
		...
		final int blockCount = (int) PackedInts.Format.PACKED.byteCount(
