...
// maybe merge with BitVector?  Problem is BitVector
...
public final class FixedBitSet extends DocIdSet implements Bits 
{
    ...
    public void or(DocIdSetIterator iter) throws IOException 
    {
        if (iter instanceof OpenBitSetIterator && iter.docID() == -1) 
        {
            ...
            // advance after last doc that would be accepted if standard iteration is used (to exhaust it):
            obs.advance(numBits);
        } 
        else 
        {
    ...
    // NOTE: no .isEmpty() here because that's trappy 
    // (ie, typically isEmpty is low cost, but this one wouldn't be)
