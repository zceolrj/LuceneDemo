...
public class CloseableThreadLocal<T> implements Closeable 
{
    ...
    // On each get or set we decrement this; when it hits 0 we purge.  
    // After purge, we set this to PURGE_MULTIPLIER * stillAliveCount.  
    // This keeps amortized cost of purging linear.
    ...
    public void close() 
    {
        // Clear the hard refs; then, the only remaining refs to all values we were storing are weak 
        // (unless somewhere else is still using them) and so GC may reclaim them:
        hardRefs = null;
        // Take care of the current thread right now; others will be taken care of via the WeakReferences.
        if (t != null) 
        {
            t.remove();
        }
