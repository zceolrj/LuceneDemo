 ...
 * labels are UTF8 bytes (or Unicode code points if unicodeArcs is true) from
 * the {@link TermToBytesRefAttribute}. Between tokens we insert POS_SEP and for
 * holes we insert HOLE.
 *
 * @lucene.experimental
 */
public class TokenStreamToAutomaton
{
	...
	/**
	 * Subclass & implement this if you need to change the token (such as
	 * escaping certain bytes) before it's turned into a graph.
	 */
	...
	/**
	 * Pulls the graph (including {@link PositionLengthAttribute}) from the
	 * provided {@link TokenStream}, and creates the corresponding automaton
	 * where arcs are bytes (or Unicode code points if unicodeArcs = true) from
	 * each term.
	 */
	public Automaton toAutomaton(TokenStream in) throws IOException
	{
		...
		final TermToBytesRefAttribute termBytesAtt = in
				.addAttribute(TermToBytesRefAttribute.class);
		final PositionIncrementAttribute posIncAtt = in
				.addAttribute(PositionIncrementAttribute.class);
		final PositionLengthAttribute posLengthAtt = in
				.addAttribute(PositionLengthAttribute.class);
		final OffsetAttribute offsetAtt = in
		...
		while (in.incrementToken())
		{
			...
			if (posInc > 0)
			{
				...
				{
					posData.leaving = new State();
					posData.arriving.addTransition(new Transition(POS_SEP,
							posData.leaving));
					if (posInc > 1)
					{
			...
			{
				// note: this isn't necessarily true. its just that we aren't
				// surely det.
				// we could optimize this further (e.g. buffer and sort synonyms
				// at a position)
			...
			{
				final State nextState = byteIDX == termLen - 1 ? endPosData.arriving
						: new State();
				int c;
				if (unicodeArcs)
				{
	...
	// for debugging!
	/*
	 * private static void toDot(Automaton a) throws IOException { final String
	 * s = a.toDot(); Writer w = new OutputStreamWriter(new
	 * FileOutputStream("/tmp/out.dot")); w.write(s); w.close();
	 * System.out.println("TEST: saved to /tmp/out.dot"); }
	 */

	private static void addHoles(State startState,
			RollingBuffer<Position> positions, int pos)
	{
		...
		while (posData.arriving == null || prevPosData.leaving == null)
		{
			...
			if (prevPosData.leaving == null)
			{
				...
				{
					prevPosData.arriving.addTransition(new Transition(POS_SEP,
							prevPosData.leaving));
				}
			}
			prevPosData.leaving.addTransition(new Transition(HOLE,
					posData.arriving));
			pos--;
			if (pos <= 0)
			{
