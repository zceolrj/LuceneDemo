 ...
 * <p>
 * NOTE: this uses the same format as {@link Lucene42DocValuesFormat} Numeric
 * DocValues, but with different file extensions, and passing
 * {@link PackedInts#FASTEST} for uncompressed encoding: trading off space for
 * performance.
 * <p>
 * Files:
 * <ul>
...
public class Lucene42NormsFormat extends NormsFormat
{
	...
	{
		// note: we choose FASTEST here (otherwise our norms are half as big but
		// 15% slower than previous lucene)
	 ...
	 *            Currently this is only used when the number of unique values
	 *            is small.
	 * 
	 * @lucene.experimental
	 */
	...
	public DocValuesConsumer normsConsumer(SegmentWriteState state)
			throws IOException
	{
		return new Lucene42NormsConsumer(state, DATA_CODEC, DATA_EXTENSION,
				METADATA_CODEC, METADATA_EXTENSION, acceptableOverheadRatio);
	}
	...
	public DocValuesProducer normsProducer(SegmentReadState state)
			throws IOException
	{
		return new Lucene42DocValuesProducer(state, DATA_CODEC, DATA_EXTENSION,
