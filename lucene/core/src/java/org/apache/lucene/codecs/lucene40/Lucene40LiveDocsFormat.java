 ...
 * </p>
 * <p>
 * Bits contains one bit for each document indexed. When the bit corresponding
 * to a document number is cleared, that document is marked as deleted. Bit
 * ordering is from least to most significant. Thus, if Bits contains two bytes,
 * 0x00 and 0x02, then document 9 is marked as alive (not deleted).
 * </p>
 * <p>
 * DGaps represents sparse bit-vectors more efficiently than Bits. It is made of
 * DGaps on indexes of nonOnes bytes in Bits, and the nonOnes bytes themselves.
 * The number of nonOnes bytes in Bits (NonOnesBytesCount) is not stored.
 * </p>
 * <p>
 * For example, if there are 8000 bits and only bits 10,12,32 are cleared, DGaps
 * would be used:
 * </p>
 * <p>
...
public class Lucene40LiveDocsFormat extends LiveDocsFormat
{
	...
	@Override
	public Bits readLiveDocs(Directory dir, SegmentInfoPerCommit info,
			IOContext context) throws IOException
	{
		...
		assert liveDocs.count() == info.info.getDocCount() - info.getDelCount() : "liveDocs.count()="
				+ liveDocs.count()
				+ " info.docCount="
	...
	public void writeLiveDocs(MutableBits bits, Directory dir,
			SegmentInfoPerCommit info, int newDelCount, IOContext context)
			throws IOException
	{
		String filename = IndexFileNames.fileNameFromGeneration(info.info.name,
				DELETES_EXTENSION, info.getNextDelGen());
	...
	@Override
	public void files(SegmentInfoPerCommit info, Collection<String> files)
			throws IOException
	{
		if (info.hasDeletions())
		{
			files.add(IndexFileNames.fileNameFromGeneration(info.info.name,
					DELETES_EXTENSION, info.getDelGen()));
		}
	}
}
