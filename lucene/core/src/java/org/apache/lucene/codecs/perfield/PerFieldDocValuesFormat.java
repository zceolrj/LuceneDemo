 ...
 * <p>
 * Note, when extending this class, the name ({@link #getName}) is written into
 * the index. In order for the field to be read, the name must resolve to your
...
public abstract class PerFieldDocValuesFormat extends DocValuesFormat
{
	 ...
	 * field.
	 */
	public static final String PER_FIELD_FORMAT_KEY = PerFieldDocValuesFormat.class
			.getSimpleName() + ".format";

	/**
	 * {@link FieldInfo} attribute name used to store the segment suffix name for
	 * each field.
	 */
	public static final String PER_FIELD_SUFFIX_KEY = PerFieldDocValuesFormat.class
	...
	private class FieldsWriter extends DocValuesConsumer
	{
		...
		public void addSortedSetField(FieldInfo field,
				Iterable<BytesRef> values, Iterable<Number> docToOrdCount,
				Iterable<Number> ords) throws IOException
		{
			getInstance(field).addSortedSetField(field, values, docToOrdCount,
					ords);
		}
				...
				throws IOException
		{
			...
			String previousValue = field.putAttribute(PER_FIELD_FORMAT_KEY,
					formatName);
			assert previousValue == null : "formatName=" + formatName
			...
			if (consumer == null)
			{
				...
				consumer.consumer = format
						.fieldsConsumer(new SegmentWriteState(
								segmentWriteState, segmentSuffix));
			...
			previousValue = field.putAttribute(PER_FIELD_SUFFIX_KEY,
	...
	static String getFullSegmentSuffix(String fieldName,
		...
		else
		{
			...
			throw new IllegalStateException(
					"cannot embed PerFieldPostingsFormat inside itself (field \""
							+ fieldName + "\" returned PerFieldPostingsFormat)");
		}
	}
	...
	private class FieldsReader extends DocValuesProducer
	{
			...
			try
			{
				...
				for (FieldInfo fi : readState.fieldInfos)
				{
					if (fi.hasDocValues())
					{
						...
						final String formatName = fi
								.getAttribute(PER_FIELD_FORMAT_KEY);
						if (formatName != null)
						{
							// null formatName means the field is in fieldInfos,
							// but has no docvalues!
							final String suffix = fi
									.getAttribute(PER_FIELD_SUFFIX_KEY);
							assert suffix != null;
							DocValuesFormat format = DocValuesFormat
									.forName(formatName);
							String segmentSuffix = getSuffix(formatName, suffix);
							if (!formats.containsKey(segmentSuffix))
							{
								formats.put(segmentSuffix, format
										.fieldsProducer(new SegmentReadState(
		...
		private FieldsReader(FieldsReader other)
		{
			...
			for (Map.Entry<String, DocValuesProducer> ent : other.formats
					.entrySet())
			{
			...
			// Then rebuild fields:
			for (Map.Entry<String, DocValuesProducer> ent : other.fields
					.entrySet())
			{
		...
		{
			long size = 0;
			for (Map.Entry<String, DocValuesProducer> entry : formats
					.entrySet())
			{
