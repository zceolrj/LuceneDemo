 ...
 * statistics (such as docfreq) and per-term metadata (typically pointers to the
 * postings list for that term in the inverted index).
 * </p>
 *
 * <p>
 * The .tim is arranged in blocks: with blocks containing a variable number of
 * entries (by default 25-48), where each entry is either a term or a reference
 * to a sub-block.
 * </p>
 *
 * <p>
 ...
 * the Postings Metadata and Term Metadata sections.
 * </p>
 *
 * <ul>
 * <!-- TODO: expand on this, its not really correct and doesnt explain
 * sub-blocks etc -->
 * <li>TermsDict (.tim) --&gt; Header, <i>Postings Metadata</i>,
 ...
 * <li>StatsBlock --&gt; StatsLength, &lt;DocFreq,
 * TotalTermFreq&gt;<sup>EntryCount</sup></li>
 * <li>MetadataBlock --&gt; MetaLength, &lt;<i>Term
 * Metadata</i>&gt;<sup>EntryCount</sup></li>
 * <li>FieldSummary --&gt; NumFields, &lt;FieldNumber, NumTerms, RootCodeLength,
 * Byte<sup>RootCodeLength</sup>, SumDocFreq, DocCount&gt;<sup>NumFields</sup></li>
 ...
 * information for the BlockTree implementation.</li>
 * <li>DirOffset is a pointer to the FieldSummary section.</li>
 * <li>DocFreq is the count of documents which contain the term.</li>
 * <li>TotalTermFreq is the total number of occurrences of the term. This is
 * encoded as the difference between the total number of occurrences and the
 * DocFreq.</li>
 ...
 * <li>SumDocFreq is the total number of postings, the number of term-document
 * pairs across the entire field.</li>
 * <li>DocCount is the number of documents that have at least one posting for
 * this field.</li>
 * <li>PostingsMetadata and TermMetadata are plugged into by the specific
 ...
 * inverted files).
 * </ul>
 * <a name="Termindex" id="Termindex"></a> <h3>Term Index</h3>
 * <p>
 * The .tip file contains an index into the term dictionary, so that it can be
 * accessed randomly. The index is also used to determine when a given term
 * cannot exist on disk (in the .tim file), saving a disk seek.
 * </p>
 * <ul>
 ...
 * </p>
 * <ul>
 * <li>The .tip file contains a separate FST for each field. The FST maps a term
 * prefix to the on-disk block that holds all terms starting with that prefix.
 * Each field's IndexStartFP points to its FST.</li>
 * <li>DirOffset is a pointer to the start of the IndexStartFPs for all fields</li>
 * <li>It's possible that an on-disk block would contain too many terms (more
 * than the allowed maximum (default: 48)). When this happens, the block is
 * sub-divided into new blocks (called "floor blocks"), and then the output in
 * the FST for the block's prefix encodes the leading byte of each sub-block,
 * and its file pointer.
 * </ul>
 *
...
public class BlockTreeTermsWriter extends FieldsConsumer
{
	...
	private static class FieldMetaData
	{
		...
		public FieldMetaData(FieldInfo fieldInfo, BytesRef rootCode,
				long numTerms, long indexStartFP, long sumTotalTermFreq,
				long sumDocFreq, int docCount)
		{
			assert numTerms > 0;
			this.fieldInfo = fieldInfo;
			assert rootCode != null : "field=" + fieldInfo.name + " numTerms="
					+ numTerms;
	...
	public BlockTreeTermsWriter(SegmentWriteState state,
			PostingsWriterBase postingsWriter, int minItemsInBlock,
			int maxItemsInBlock) throws IOException
	{
		...
		try
		{
			...
			final String termsIndexFileName = IndexFileNames.segmentFileName(
					state.segmentInfo.name, state.segmentSuffix,
					TERMS_INDEX_EXTENSION);
			indexOut = state.directory.createOutput(termsIndexFileName,
					state.context);
	...
	{
		CodecUtil.writeHeader(out, TERMS_INDEX_CODEC_NAME,
	...
	public TermsConsumer addField(FieldInfo field) throws IOException
	{
		...
		// if (DEBUG) System.out.println("\nBTTW.addField seg=" + segment +
		// " field=" + field.name);
		assert currentField == null
				|| currentField.name.compareTo(field.name) < 0;
		currentField = field;
	...
	private static final class PendingBlock extends PendingEntry
	{
		...
		public PendingBlock(BytesRef prefix, long fp, boolean hasTerms,
				boolean isFloor, int floorLeadByte,
		...
		public void compileIndex(List<PendingBlock> floorBlocks,
				RAMOutputStream scratchBytes) throws IOException
		{

			assert (isFloor && floorBlocks != null && floorBlocks.size() != 0)
					|| (!isFloor && floorBlocks == null) : "isFloor=" + isFloor
			...
			if (isFloor)
			{
				...
				for (PendingBlock sub : floorBlocks)
				{
					...
					assert sub.fp > fp;
					scratchBytes.writeVLong((sub.fp - fp) << 1
							| (sub.hasTerms ? 1 : 0));
				}
			}
			...
			final Builder<BytesRef> indexBuilder = new Builder<BytesRef>(
					...
					outputs, null, false, PackedInts.COMPACT, true, 15);
			// if (DEBUG) {
			 ...
			 * FileOutputStream("out.dot")); Util.toDot(index, w, false, false);
		...
		private void append(Builder<BytesRef> builder, FST<BytesRef> subIndex)
				throws IOException
		{
			final BytesRefFSTEnum<BytesRef> subIndexEnum = new BytesRefFSTEnum<BytesRef>(
					subIndex);
	...
	class TermsWriter extends TermsConsumer
	{
		...
		private class FindBlocks extends Builder.FreezeTail<Object>
		{
			...
			@Override
			public void freeze(final Builder.UnCompiledNode<Object>[] frontier,
					int prefixLenPlus1, final IntsRef lastInput)
					throws IOException
			{

				// if (DEBUG) System.out.println("  freeze prefixLenPlus1=" +
				// prefixLenPlus1);
				...
				for (int idx = lastInput.length; idx >= prefixLenPlus1; idx--)
				{
					...
					for (int arcIdx = 0; arcIdx < node.numArcs; arcIdx++)
					{
						...
						final Builder.UnCompiledNode<Object> target = (Builder.UnCompiledNode<Object>) node.arcs[arcIdx].target;
					...
					frontier[idx] = new Builder.UnCompiledNode<Object>(
							blockBuilder, idx);
				}
			}
		}
				...
				throws IOException
		{
			if (prefixLength == 0 || count <= maxItemsInBlock)
			{
				...
				// maxItemsInBlock.
				final PendingBlock nonFloorBlock = writeBlock(prevTerm,
						prefixLength, prefixLength, count, count, 0, false, -1,
						true);
			...
			else
			{
				...
				// //System.out.println("\nWBS count=" + count + " prefix=" +
				// prefix.utf8ToString() + " " + prefix);
				// System.out.println("writeBlocks: prefix=" + prefix + " " +
				// prefix + " count=" + count + " pending.size()=" +
				// pending.size());
				// }
				// System.out.println("\nwbs count=" + count);

				final int savLabel = prevTerm.ints[prevTerm.offset
						+ prefixLength];
				...
				final List<PendingEntry> slice = pending.subList(pending.size()
						- count, pending.size());
				...
				for (PendingEntry ent : slice)
				{
					...
					if (ent.isTerm)
					{
						...
						{
							suffixLeadLabel = term.term.bytes[term.term.offset
									+ prefixLength] & 0xff;
						}
					}
					else
					{
						...
						suffixLeadLabel = block.prefix.bytes[block.prefix.offset
								+ prefixLength] & 0xff;
					}
							...
							&& (termCount + subCount) != 0)
					{
						 ...
						 * System.out.println("  sub " + -1 + " termCount=" +
						 * termCount + " subCount=" + subCount); } else {
						 * System.out.println("  sub " +
						 * Integer.toHexString(suffixLeadLabel) + " termCount="
						 * + termCount + " subCount=" + subCount); }
						 */
						termCount = subCount = 0;
						numSubs++;
					}
				...
				numSubs++;
				/*
				 * if (lastSuffixLeadLabel == -1) { System.out.println("  sub "
				 * + -1 + " termCount=" + termCount + " subCount=" + subCount);
				 * } else { System.out.println("  sub " +
				 * Integer.toHexString(lastSuffixLeadLabel) + " termCount=" +
				 * termCount + " subCount=" + subCount); }
				 */

				if (subTermCountSums.length < numSubs)
				{
					subTermCountSums = ArrayUtil
							.grow(subTermCountSums, numSubs);
				}
				...
				for (int sub = 0; sub < numSubs; sub++)
				{
					...
					// System.out.println("  " + (subTermCounts[sub] +
					// subSubCounts[sub]));
					subCount++;
					...
					if (pendingCount >= minItemsInBlock)
					{
						...
						final PendingBlock floorBlock = writeBlock(prevTerm,
								prefixLength, curPrefixLength, curStart,
								pendingCount, subTermCountSums[1 + sub], true,
						...
						assert minItemsInBlock == 1 || subCount > 1 : "minItemsInBlock="
								+ minItemsInBlock
								+ " subCount="
								+ subCount
								+ " sub="
						...
						if (curStart <= maxItemsInBlock)
						{
							...
							prevTerm.ints[prevTerm.offset + prefixLength] = startLabel;
							// System.out.println("  final " + (numSubs-sub-1) +
							// " subs");
							/*
							 * for(sub++;sub < numSubs;sub++) {
							 * System.out.println("  " + (subTermCounts[sub] +
							 * subSubCounts[sub])); }
							 * System.out.println("    = " + curStart); if
							 * (curStart < minItemsInBlock) {
							 * System.out.println("      **"); }
							 */
							floorBlocks.add(writeBlock(prevTerm, prefixLength,
									prefixLength + 1, curStart, curStart, 0,
									true, startLabel, true));
							break;
						}
					}
				}
				...
				// if (DEBUG) System.out.println("  done pending.size()=" +
				// pending.size());
			}
		...
		// block:
		private PendingBlock writeBlock(IntsRef prevTerm, int prefixLength,
				int indexPrefixLength, int startBackwards, int length,
				int futureTermCount, boolean isFloor, int floorLeadByte,
				boolean isLastInFloor) throws IOException
		{
			...
			assert start >= 0 : "pending.size()=" + pending.size()
					+ " startBackwards=" + startBackwards + " length=" + length;

			final List<PendingEntry> slice = pending.subList(start, start
					+ length);
			...
			// if (DEBUG) {
			// System.out.println("  writeBlock " + (isFloor ? "(floor) " : "")
			// + "seg=" + segment + " pending.size()=" + pending.size() +
			// " prefixLength=" + prefixLength + " indexPrefix=" +
			// toString(prefix) + " entCount=" + length + " startFP=" + startFP
			// + " futureTermCount=" + futureTermCount + (isFloor ?
			// (" floorLeadByte=" + Integer.toHexString(floorLeadByte&0xff)) :
			...
			else if (!isFloor)
			{
				...
				isLeafBlock = false;
				// System.out.println("no scan false " + lastBlockIndex +
				// " vs start=" + start + " len=" + length);
			}
			else
			{
				// Must scan up-front to see if there is a sub-block
				boolean v = true;
				// System.out.println("scan " + lastBlockIndex + " vs start=" +
				// start + " len=" + length);
				for (PendingEntry ent : slice)
				{
					if (!ent.isTerm)
			...
			if (isLeafBlock)
			{
				...
				for (PendingEntry ent : slice)
				{
					...
					// System.arraycopy(term.term.bytes, prefixLength,
					// suffixBytes.bytes, 0, suffix);
					...
					bytesWriter.writeBytes(term.term.bytes, prefixLength,
							suffix);
					...
					{
						assert term.stats.totalTermFreq >= term.stats.docFreq : term.stats.totalTermFreq
								+ " vs " + term.stats.docFreq;
						bytesWriter2.writeVLong(term.stats.totalTermFreq
								- term.stats.docFreq);
					}
				}
				termCount = length;
			...
			else
			{
				...
				for (PendingEntry ent : slice)
				{
					...
					else
					{
						...
						bytesWriter.writeBytes(block.prefix.bytes,
								prefixLength, suffix);
						...
						// toString(suffixBytes) + " subFP=" + block.fp +
						// " subCode=" + (startFP-block.fp) + " floor=" +
						// block.isFloor);
						// }
			...
			out.writeVInt((int) (bytesWriter.getFilePointer() << 1)
					| (isLeafBlock ? 1 : 0));
			...
			postingsWriter.flushTermsBlock(futureTermCount + termCount,
					termCount);
			...
			return new PendingBlock(prefix, startFP, termCount != 0, isFloor,
					floorLeadByte, subIndices);
		}
		...
		TermsWriter(FieldInfo fieldInfo)
		{
			...
			// resulting FST:
			blockBuilder = new Builder<Object>(FST.INPUT_TYPE.BYTE1, 0, 0,
					true, true, Integer.MAX_VALUE, noOutputs, new FindBlocks(),
					false, PackedInts.COMPACT, true, 15);
		...
		{
			// if (DEBUG) System.out.println("\nBTTW.startTerm term=" +
			// fieldInfo.name + ":" + toString(text) + " seg=" + segment);
			postingsWriter.startTerm();
			/*
			 * if (fieldInfo.name.equals("id")) { postingsWriter.termID =
			 * Integer.parseInt(text.utf8ToString()); } else {
				...
				throws IOException
		{
			...
			// if (DEBUG) System.out.println("BTTW.finishTerm term=" +
			// fieldInfo.name + ":" + toString(text) + " seg=" + segment +
			// " df=" + stats.docFreq);

			blockBuilder.add(Util.toIntsRef(text, scratchIntsRef),
					noOutputs.getNoOutput());
				...
				throws IOException
		{
			if (numTerms > 0)
			{
				...
				assert pending.size() == 1 && !pending.get(0).isTerm : "pending.size()="
						+ pending.size() + " pending=" + pending;
				...
				fields.add(new FieldMetaData(fieldInfo, ((PendingBlock) pending
						.get(0)).index.getEmptyOutput(), numTerms,
						indexStartFP, sumTotalTermFreq, sumDocFreq, docCount));
			}
			else
			{
	...
	public void close() throws IOException
	{
		...
		try
		{
			...
			for (FieldMetaData field : fields)
			{
				...
				out.writeBytes(field.rootCode.bytes, field.rootCode.offset,
		...
		{
			IOUtils.closeWhileHandlingException(ioe, out, indexOut,
					postingsWriter);
		}
	}
}
