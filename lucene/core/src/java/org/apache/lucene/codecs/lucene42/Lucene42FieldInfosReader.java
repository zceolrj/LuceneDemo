...
final class Lucene42FieldInfosReader extends FieldInfosReader
{
			...
			IOContext iocontext) throws IOException
	{
		...
		try
		{
			...
			for (int i = 0; i < size; i++)
			{
				...
				final DocValuesType docValuesType = getDocValuesType(input,
						(byte) (val & 0x0F));
				final DocValuesType normsType = getDocValuesType(input,
						(byte) ((val >>> 4) & 0x0F));
				final Map<String, String> attributes = input
						.readStringStringMap();
				infos[i] = new FieldInfo(name, isIndexed, fieldNumber,
						storeTermVector, omitNorms, storePayloads,
						indexOptions, docValuesType, normsType,
			...
			throws IOException
	{
		...
		{
			throw new CorruptIndexException("invalid docvalues byte: " + b
					+ " (resource=" + input + ")");
		}
	}
}
