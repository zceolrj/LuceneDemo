...
final class LZ4
{
	...
	static final int LAST_LITERALS = 5; // the last 5 bytes must be encoded as
										// literals
	static final int HASH_LOG_HC = 15; // log size of the dictionary for
										// compressHC
	static final int HASH_TABLE_SIZE_HC = 1 << HASH_LOG_HC;
	static final int OPTIMAL_ML = 0x0F + 4 - 1; // match length that doesn't
	...
	private static int commonBytesBackward(byte[] b, int o1, int o2, int l1,
			int l2)
	{
		int count = 0;
	 ...
	 * <code>dest[dOff:]</code>. Please note that <code>dest</code> must be
	 * large enough to be able to hold <b>all</b> decompressed data (meaning
	 * that you need to know the total decompressed length).
	 */
	public static int decompress(DataInput compressed, int decompressedLen,
			byte[] dest, int dOff) throws IOException
	{
		...
		do
		{
			...
			// copying a multiple of 8 bytes can make decompression from 5% to
			// 10% faster
	...
	private static void encodeLiterals(byte[] bytes, int token, int anchor,
			int literalLen, DataOutput out) throws IOException
	{
	...
	private static void encodeLastLiterals(byte[] bytes, int anchor,
			int literalLen, DataOutput out) throws IOException
	{
	...
	private static void encodeSequence(byte[] bytes, int anchor, int matchRef,
			int matchOff, int matchLen, DataOutput out) throws IOException
	{
		final int literalLen = matchOff - anchor;
		assert matchLen >= 4;
		// encode token
		final int token = (Math.min(literalLen, 0x0F) << 4)
				| Math.min(matchLen - 4, 0x0F);
		encodeLiterals(bytes, token, anchor, literalLen, out);
	...
	static final class HashTable
	{
		...
		void reset(int len)
		{
			final int bitsPerOffset = PackedInts.bitsRequired(len
					- LAST_LITERALS);
			final int bitsPerOffsetLog = 32 - Integer
					.numberOfLeadingZeros(bitsPerOffset - 1);
			hashLog = MEMORY_USAGE + 3 - bitsPerOffsetLog;
			if (hashTable == null || hashTable.size() < 1 << hashLog
	...
	/**
	 * Compress <code>bytes[off:off+len]</code> into <code>out</code> using at
	 * most 16KB of memory. <code>ht</code> shouldn't be shared across threads
	 * but can safely be reused.
	 */
	public static void compress(byte[] bytes, int off, int len, DataOutput out,
			HashTable ht) throws IOException
	{
		final int base = off;
		...
		if (len > LAST_LITERALS + MIN_MATCH)
		{
			...
			main: while (off <= limit)
			{
				...
				while (true)
				{
					...
					assert PackedInts.bitsRequired(off - base) <= hashTable
							.getBitsPerValue();
				...
				final int matchLen = MIN_MATCH
						+ commonBytes(bytes, ref + MIN_MATCH, off + MIN_MATCH,
								limit);
	...
	static final class HCHashTable
	{
		...
		boolean insertAndFindBestMatch(byte[] buf, int off, int matchLimit,
				Match match)
		{
			match.start = off;
			match.len = 0;
		...
		boolean insertAndFindWiderMatch(byte[] buf, int off, int startLimit,
				int matchLimit, int minLen, Match match)
		{
			match.len = minLen;
			...
			for (int i = 0; i < MAX_ATTEMPTS; ++i)
			{
						...
						&& readIntEquals(buf, ref, off))
				{
					final int matchLenForward = MIN_MATCH
							+ commonBytes(buf, ref + MIN_MATCH,
									off + MIN_MATCH, matchLimit);
					final int matchLenBackward = commonBytesBackward(buf, ref,
							off, base, startLimit);
	...
	/**
	 * Compress <code>bytes[off:off+len]</code> into <code>out</code>. Compared
	 * to {@link LZ4#compress(byte[], int, int, DataOutput, HashTable)}, this
	 * method is slower and uses more memory (~ 256KB per thread) but should
	 * provide better compression ratios (especially on large inputs) because it
	 * chooses the best match among up to 256 candidates and then performs
	 * trade-offs to fix overlapping matches. <code>ht</code> shouldn't be
	 * shared across threads but can safely be reused.
	 */
	public static void compressHC(byte[] src, int srcOff, int srcLen,
			DataOutput out, HCHashTable ht) throws IOException
	{
			...
			search2: while (true)
			{
				...
				if (match1.end() >= mfLimit
						|| !ht.insertAndFindWiderMatch(src, match1.end() - 2,
								match1.start + 1, matchLimit, match1.len,
								match2))
				{
					// no better match
					encodeSequence(src, anchor, match1.ref, match1.start,
							match1.len, out);
				...
				search3: while (true)
				{
					if (match2.start - match1.start < OPTIMAL_ML)
					{
								...
								- MIN_MATCH)
						{
							newMatchLen = match2.start - match1.start
									+ match2.len - MIN_MATCH;
						}
						final int correction = newMatchLen
								- (match2.start - match1.start);
						if (correction > 0)
						{
					...
					if (match2.start + match2.len >= mfLimit
							|| !ht.insertAndFindWiderMatch(src,
									match2.end() - 3, match2.start, matchLimit,
									match2.len, match3))
					{
						...
						anchor = sOff = match1.end();
						// encode seq 2
						encodeSequence(src, anchor, match2.ref, match2.start,
								match2.len, out);
					...
					{ // Not enough space for match 2 : remove it
						...
						{ // // can write Seq1 immediately ==> Seq2 is removed,
							...
							if (match2.start < match1.end())
							{
								final int correction = match1.end()
										- match2.start;
								match2.fix(correction);
							...
							encodeSequence(src, anchor, match1.ref,
									match1.start, match1.len, out);
							anchor = sOff = match1.end();
					...
					if (match2.start < match1.end())
					{
						if (match2.start - match1.start < 0x0F)
						{
							...
							{
								match1.len = match2.end() - match1.start
										- MIN_MATCH;
							}
