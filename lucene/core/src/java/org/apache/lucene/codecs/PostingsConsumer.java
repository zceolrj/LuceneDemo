 ...
 * <li>{@link #startDoc(int, int)} is called for each document where the term
 * occurs, specifying id and term frequency for that document.
 * <li>If positions are enabled for the field, then
 * {@link #addPosition(int, BytesRef, int, int)} will be called for each
 * occurrence in the document.
 * <li>{@link #finishDoc()} is called when the producer is done adding positions
 * to the document.
 * </ol>
 * 
 * @lucene.experimental
...
public abstract class PostingsConsumer
{
	...
	/**
	 * Adds a new doc in this term. <code>freq</code> will be -1 when term
	 * frequencies are omitted for the field.
	 */
	public abstract void startDoc(int docID, int freq) throws IOException;

	/**
	 * Add a new position & payload, and start/end offset. A null payload means
	 * no payload; a non-null payload with zero length also means no payload.
	 * Caller may reuse the {@link BytesRef} for the payload between calls
	 * (method must fully consume the payload). <code>startOffset</code> and
	 * <code>endOffset</code> will be -1 when offsets are not indexed.
	 */
	 ...
	 */
	public TermStats merge(final MergeState mergeState,
			IndexOptions indexOptions, final DocsEnum postings,
