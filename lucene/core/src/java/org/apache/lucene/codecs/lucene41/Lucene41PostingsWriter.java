...
/**
 * Concrete class that writes docId(maybe frq,pos,offset,payloads) list with
 * postings format.
 *
 * Postings list for each term will be stored separately.
 *
 * @see Lucene41SkipWriter for details about skipping setting and postings
 *      layout.
 * @lucene.experimental
 */
public final class Lucene41PostingsWriter extends PostingsWriterBase
{
			...
			float acceptableOverheadRatio) throws IOException
	{
		...
		docOut = state.directory.createOutput(IndexFileNames.segmentFileName(
				state.segmentInfo.name, state.segmentSuffix,
				Lucene41PostingsFormat.DOC_EXTENSION), state.context);
		...
		try
		{
			...
			if (state.fieldInfos.hasProx())
			{
				...
				posOut = state.directory.createOutput(IndexFileNames
						.segmentFileName(state.segmentInfo.name,
								state.segmentSuffix,
				...
				if (state.fieldInfos.hasPayloads()
						|| state.fieldInfos.hasOffsets())
				{
					payOut = state.directory.createOutput(IndexFileNames
							.segmentFileName(state.segmentInfo.name,
									state.segmentSuffix,
		...
		skipWriter = new Lucene41SkipWriter(maxSkipLevels, BLOCK_SIZE,
				state.segmentInfo.getDocCount(), docOut, posOut, payOut);
	...
	public void startDoc(int docID, int termDocFreq) throws IOException
	{
		...
		{
			// if (DEBUG) {
			// System.out.println("  bufferSkip at writeBlock: lastDocID=" +
			// lastBlockDocID + " docCount=" + (docCount-1));
			// }
		...
		if (docID < 0 || (docCount > 0 && docDelta <= 0))
		{
			throw new CorruptIndexException("docs out of order (" + docID
					+ " <= " + lastDocID + " ) (docOut: " + docOut + ")");
		}

		docDeltaBuffer[docBufferUpto] = docDelta;
		// if (DEBUG) {
		// System.out.println("  docDeltaBuffer[" + docBufferUpto + "]=" +
		// docDelta);
		// }
		if (fieldHasFreqs)
		{
		...
		{
			// if (DEBUG) {
			// System.out.println("  write docDelta block @ fp=" +
			// docOut.getFilePointer());
			// }
			forUtil.writeBlock(docDeltaBuffer, encoded, docOut);
			if (fieldHasFreqs)
			{
				// if (DEBUG) {
				// System.out.println("  write freq block @ fp=" +
				// docOut.getFilePointer());
				// }
	...
	{
		// if (DEBUG) {
		// System.out.println("FPW.addPosition pos=" + position +
		// " posBufferUpto=" + posBufferUpto + (fieldHasPayloads ?
		// " payloadByteUpto=" + payloadByteUpto: ""));
		// }
		...
		if (fieldHasOffsets)
		{
			...
			offsetStartDeltaBuffer[posBufferUpto] = startOffset
					- lastStartOffset;
		...
		{
			// if (DEBUG) {
			// System.out.println("  write pos bulk block @ fp=" +
			// posOut.getFilePointer());
			// }
	...
	public void finishDoc() throws IOException
	{
		...
		if (docBufferUpto == BLOCK_SIZE)
		{
			...
			// if (DEBUG) {
			// System.out.println("  docBufferUpto="+docBufferUpto+" now get lastBlockDocID="+lastBlockDocID+" lastBlockPosFP="
			// + lastBlockPosFP + " lastBlockPosBufferUpto=" +
	...
	private static class PendingTerm
	{
		...
		public PendingTerm(long docStartFP, long posStartFP, long payStartFP,
				long skipOffset, long lastPosBlockOffset, int singletonDocID)
		{
	...
	public void finishTerm(TermStats stats) throws IOException
	{
		...
		// if (DEBUG) {
		// if (docBufferUpto > 0) {
		// System.out.println("  write doc/freq vInt block (count=" +
		// docBufferUpto + ") at fp=" + docOut.getFilePointer() +
		// " docTermStartFP=" + docTermStartFP);
		// }
		// }

		// docFreq == 1, don't write the single docid/freq to a separate file
		// along with a pointer to it.
		final int singletonDocID;
		if (stats.docFreq == 1)
		{
			// pulse the singleton docid into the term dictionary, freq is
			// implicitly totalTermFreq
		...
		if (fieldHasPositions)
		{
			// if (DEBUG) {
			// if (posBufferUpto > 0) {
			// System.out.println("  write pos vInt block (count=" +
			// posBufferUpto + ") at fp=" + posOut.getFilePointer() +
			// " posTermStartFP=" + posTermStartFP + " hasPayloads=" +
			// fieldHasPayloads + " hasOffsets=" + fieldHasOffsets);
			// }
			// }
			...
			if (posBufferUpto > 0)
			{
				...
				int lastPayloadLength = -1; // force first payload length to be
											// written
				int lastOffsetLength = -1; // force first offset length to be
											// written
				...
				for (int i = 0; i < posBufferUpto; i++)
				{
					...
					if (fieldHasPayloads)
					{
						...
						// if (DEBUG) {
						// System.out.println("        i=" + i + " payloadLen="
						// + payloadLength);
						// }
						...
						{
							// if (DEBUG) {
							// System.out.println("          write payload @ pos.fp="
							// + posOut.getFilePointer());
							// }
							posOut.writeBytes(payloadBytes,
					...
					{
						// if (DEBUG) {
						// System.out.println("          write offset @ pos.fp="
						// + posOut.getFilePointer());
						// }
			...
			// if (DEBUG) {
			// System.out.println("  totalTermFreq=" + stats.totalTermFreq +
			// " lastPosBlockOffset=" + lastPosBlockOffset);
			// }
		}
		else
		{
		...
		if (docCount > BLOCK_SIZE)
		{
			...
			// if (DEBUG) {
			// System.out.println("skip packet " + (docOut.getFilePointer() -
			// (docTermStartFP + skipOffset)) + " bytes");
			// }
		}
		else
		{
		...
		pendingTerms.add(new PendingTerm(docTermStartFP, posTermStartFP,
				payStartFP, skipOffset, lastPosBlockOffset, singletonDocID));
		docBufferUpto = 0;
		posBufferUpto = 0;
		lastDocID = 0;
		docCount = 0;
