 ...
 * <p>
 * Note, when extending this class, the name ({@link #getName}) may written into
 * the index in certain configurations. In order for the segment to be read, the
 * name must resolve to your implementation via {@link #forName(String)}. This
 * method uses Java's {@link ServiceLoader Service Provider Interface} (SPI) to
...
public abstract class DocValuesFormat implements NamedSPILoader.NamedSPI
{
	...
	private static final NamedSPILoader<DocValuesFormat> loader = new NamedSPILoader<DocValuesFormat>(
			DocValuesFormat.class);
	 ...
	 * need to use; else, those files may be deleted. Additionally, required
	 * files may be deleted during the execution of this call before there is a
	 * chance to open them. Under these circumstances an IOException should be
	 * thrown by the implementation. IOExceptions are expected and will
	 * automatically cause a retry of the segment opening logic with the newly
	 * revised segments.
	 */
	...
	public static DocValuesFormat forName(String name)
	{
		if (loader == null)
		{
			throw new IllegalStateException(
					"You called DocValuesFormat.forName() before all formats could be initialized. "
	...
	public static Set<String> availableDocValuesFormats()
	{
		...
		{
			throw new IllegalStateException(
					"You called DocValuesFormat.availableDocValuesFormats() before all formats could be initialized. "
							+ "This likely happens if you call it from a DocValuesFormat's ctor.");
		}
	 ...
	 * 
	 * <p>
	 * <b>NOTE:</b> Only new docvalues formats are added, existing ones are
