...
public class Lucene45DocValuesProducer extends DocValuesProducer implements
		Closeable
{
	...
	protected Lucene45DocValuesProducer(SegmentReadState state,
			String dataCodec, String dataExtension, String metaCodec,
			...
			throws IOException
	{
		...
		while (fieldNumber != -1)
		{
			...
			else if (type == Lucene45DocValuesFormat.SORTED)
			{
				...
				{
					throw new CorruptIndexException("sorted entry for field: "
							+ fieldNumber + " is corrupt (resource=" + meta
							+ ")");
				}
			...
			else if (type == Lucene45DocValuesFormat.SORTED_SET)
			{
				...
				if (meta.readVInt() != fieldNumber)
				{
					throw new CorruptIndexException(
							"sortedset entry for field: " + fieldNumber
									+ " is corrupt (resource=" + meta + ")");
				}
				if (meta.readByte() != Lucene45DocValuesFormat.BINARY)
				{
					throw new CorruptIndexException(
			...
			{
				throw new CorruptIndexException("invalid type: " + type
						+ ", resource=" + meta);
			}
	...
	{
		long sizeInBytes = 0;
		for (MonotonicBlockPackedReader monotonicBlockPackedReader : addressInstances
				.values())
		{
			sizeInBytes += Integer.SIZE
					+ monotonicBlockPackedReader.ramBytesUsed();
		}
		for (MonotonicBlockPackedReader monotonicBlockPackedReader : ordIndexInstances
				.values())
		{
			sizeInBytes += Integer.SIZE
					+ monotonicBlockPackedReader.ramBytesUsed();
	...
	LongNumericDocValues getNumeric(NumericEntry entry) throws IOException
	{
		...
		case GCD_COMPRESSED:
			...
			final BlockPackedReader quotientReader = new BlockPackedReader(
					data, entry.packedIntsVersion, entry.blockSize,
					entry.count, true);
			return new LongNumericDocValues()
			{
				@Override
		...
		case TABLE_COMPRESSED:
			...
			final PackedInts.Reader ords = PackedInts.getDirectReaderNoHeader(
					data, PackedInts.Format.PACKED, entry.packedIntsVersion,
					(int) entry.count, bitsRequired);
			return new LongNumericDocValues()
			{
				@Override
				public long get(long id)
	...
	private BinaryDocValues getFixedBinary(FieldInfo field,
	...
	protected MonotonicBlockPackedReader getAddressInstance(IndexInput data,
			FieldInfo field, BinaryEntry bytes) throws IOException
	{
		final MonotonicBlockPackedReader addresses;
		synchronized (addressInstances)
		{
			MonotonicBlockPackedReader addrInstance = addressInstances
					.get(field.number);
			if (addrInstance == null)
			{
				data.seek(bytes.addressesOffset);
				addrInstance = new MonotonicBlockPackedReader(data,
						bytes.packedIntsVersion, bytes.blockSize, bytes.count,
						false);
	...
	private BinaryDocValues getVariableBinary(FieldInfo field,
			final BinaryEntry bytes) throws IOException
	{
		final IndexInput data = this.data.clone();

		final MonotonicBlockPackedReader addresses = getAddressInstance(data,
				field, bytes);
			...
			FieldInfo field, BinaryEntry bytes) throws IOException
	{
		...
		synchronized (addressInstances)
		{
			...
			if (addrInstance == null)
			{
				...
				addrInstance = new MonotonicBlockPackedReader(data,
						bytes.packedIntsVersion, bytes.blockSize, size, false);
				addressInstances.put(field.number, addrInstance);
			}
			addresses = addrInstance;
		}
	...
	private BinaryDocValues getCompressedBinary(FieldInfo field,
			final BinaryEntry bytes) throws IOException
	{
		final IndexInput data = this.data.clone();

		final MonotonicBlockPackedReader addresses = getIntervalInstance(data,
				field, bytes);
	...
	public SortedDocValues getSorted(FieldInfo field) throws IOException
	{
		...
		data.seek(entry.offset);
		final BlockPackedReader ordinals = new BlockPackedReader(data,
				entry.packedIntsVersion, entry.blockSize, entry.count, true);
		...
		return new SortedDocValues()
		{
			...
			public int lookupTerm(BytesRef key)
			{
				...
				{
					return (int) ((CompressedBinaryDocValues) binary)
							.lookupTerm(key);
				}
				else
				{
	...
	protected MonotonicBlockPackedReader getOrdIndexInstance(IndexInput data,
			FieldInfo field, NumericEntry entry) throws IOException
	{
		final MonotonicBlockPackedReader ordIndex;
		synchronized (ordIndexInstances)
		{
			MonotonicBlockPackedReader ordIndexInstance = ordIndexInstances
					.get(field.number);
			if (ordIndexInstance == null)
			{
				data.seek(entry.offset);
				ordIndexInstance = new MonotonicBlockPackedReader(data,
						entry.packedIntsVersion, entry.blockSize, entry.count,
						false);
	...
	public SortedSetDocValues getSortedSet(FieldInfo field) throws IOException
	{
		...
		final MonotonicBlockPackedReader ordIndex = getOrdIndexInstance(data,
				field, ordIndexes.get(field.number));
	...
	protected static class NumericEntry
	{
		...
		/**
		 * offset to the bitset representing docsWithField, or -1 if no
		 * documents have missing values
		 */
		long missingOffset;
	...
	protected static class BinaryEntry
	{
		...
		/**
		 * interval of shared prefix chunks (when using prefix-compressed
		 * binary)
		 */
	...
	static class CompressedBinaryDocValues extends LongBinaryDocValues
	{
		...
		public CompressedBinaryDocValues(BinaryEntry bytes,
				MonotonicBlockPackedReader addresses, IndexInput data)
				throws IOException
		{
			this.bytes = bytes;
		...
		private TermsEnum getTermsEnum(final IndexInput input)
				throws IOException
		{
