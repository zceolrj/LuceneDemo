...
/**
 * Encode all values in normal area with fixed bit width, which is determined by
 * the max value in this block.
 */
final class ForUtil
{
	...
	/**
	 * Upper limit of the number of values that might be decoded in a single
	 * call to {@link #readBlock(IndexInput, byte[], int[])}. Although values
	 * after <code>BLOCK_SIZE</code> are garbage, it is necessary to allocate
	 * value buffers whose size is >= MAX_DATA_SIZE to avoid
		...
		for (int version = PackedInts.VERSION_START; version <= PackedInts.VERSION_CURRENT; version++)
		{
			for (PackedInts.Format format : PackedInts.Format.values())
			{
				for (int bpv = 1; bpv <= 32; ++bpv)
				{
					...
					final PackedInts.Decoder decoder = PackedInts.getDecoder(
							format, version, bpv);
					final int iterations = computeIterations(decoder);
					maxDataSize = Math.max(maxDataSize,
							iterations * decoder.byteValueCount());
				}
			}
		}
	 ...
	 * <code>BLOCK_SIZE</code> values with the provided {@link Decoder}.
	 */
	...
	private static int encodedSize(PackedInts.Format format,
			int packedIntsVersion, int bitsPerValue)
	{
		final long byteCount = format.byteCount(packedIntsVersion, BLOCK_SIZE,
				bitsPerValue);
	...
	/**
	 * Create a new {@link ForUtil} instance and save state into
	 * <code>out</code>.
	 */
	ForUtil(float acceptableOverheadRatio, DataOutput out) throws IOException
	{
		...
		for (int bpv = 1; bpv <= 32; ++bpv)
		{
			final FormatAndBits formatAndBits = PackedInts
					.fastestFormatAndBits(BLOCK_SIZE, bpv,
							acceptableOverheadRatio);
			assert formatAndBits.format.isSupported(formatAndBits.bitsPerValue);
			assert formatAndBits.bitsPerValue <= 32;
			encodedSizes[bpv] = encodedSize(formatAndBits.format,
					PackedInts.VERSION_CURRENT, formatAndBits.bitsPerValue);
	...
	ForUtil(DataInput in) throws IOException
	{
		...
		for (int bpv = 1; bpv <= 32; ++bpv)
		{
			...
			encodedSizes[bpv] = encodedSize(format, packedIntsVersion,
					bitsPerValue);
			encoders[bpv] = PackedInts.getEncoder(format, packedIntsVersion,
					bitsPerValue);
			decoders[bpv] = PackedInts.getDecoder(format, packedIntsVersion,
					bitsPerValue);
			iterations[bpv] = computeIterations(decoders[bpv]);
		}
	}
	 ...
	 */
	void writeBlock(int[] data, byte[] encoded, IndexOutput out)
			throws IOException
	{
		if (isAllEqual(data))
		{
