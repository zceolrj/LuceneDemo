...
class TwoStoredFieldsConsumers extends StoredFieldsConsumer 
{
    ...
    {
        first.addField(docID, field, fieldInfo);//first=StoredFieldsProcessor
        second.addField(docID, field, fieldInfo);//second=DocValuesProcessor
    }
  
    @Override
    void flush(SegmentWriteState state) throws IOException 
    {
        first.flush(state);//StoredFieldsProcessor
        second.flush(state);//DocValuesProcessor
    }
    ...
    {
        first.startDocument();//first=StoredFieldsProcessor
        second.startDocument();//second=DocValuesProcessor
    }
  
    @Override
    void finishDocument() throws IOException 
    {
        first.finishDocument();//first=StoredFieldsProcessor
        second.finishDocument();//second=DocValuesProcessor
    }
}
