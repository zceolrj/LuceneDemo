...
{  
    ...
    protected final void ensureOpen() throws AlreadyClosedException 
    {
        ...
        // the happens before rule on reading the refCount, which must be after the fake write, ensures that we see the value:
        if (closedByChild) 
        {
     ...
     * @throws IOException if there is a low-level IO error
     */
    // we need a separate StoredField, so that the
    ...
    {
        // Don't call ensureOpen since FC calls this (to evict) on close
        return this;
    }
