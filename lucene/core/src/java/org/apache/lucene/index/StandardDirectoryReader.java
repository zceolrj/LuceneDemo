...
final class StandardDirectoryReader extends DirectoryReader 
{
        ...
        finally 
        {
            if (!success) 
            {
                for (SegmentReader r : readers) 
                {
                    ...
                    {
                        // ignore any exception that is thrown here to not mask any original exception. 
                    }
                }
            }
        }
        ...
        for (int i = infos.size() - 1; i>=0; i--) 
        {
            ...
            try 
            {
                ...
                if (newReaders[i] == null || 
                    infos.info(i).info.getUseCompoundFile() != newReaders[i].getSegmentInfo().info.getUseCompoundFile()) 
                {
                ...
                else 
                {
                    if (newReaders[i].getSegmentInfo().getDelGen() == infos.info(i).getDelGen()) 
                    {
                        ...
                        // the old and the new one, so we must incRef it:
                        readerShared[i] = true;
                        newReaders[i].incRef();
                    } 
                    else 
    ...
    public boolean isCurrent() throws IOException 
    {
        ...
        if (writer == null || writer.isClosed()) 
        {
            ...
            // yet commit), then the reader will still see itself as current:
            SegmentInfos sis = new SegmentInfos();
            sis.read(directory);
