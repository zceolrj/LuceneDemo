...
// break into separate freq and prox writers as
...
final class FreqProxTermsWriterPerField extends TermsHashConsumerPerField implements Comparable<FreqProxTermsWriterPerField> 
{
    ...
    void addTerm(final int termID) 
    {
        ...
        {
            assert docState.docID > postings.lastDocIDs[termID]:"id: "+docState.docID + " postings ID: " + 
                postings.lastDocIDs[termID] + " termID: "+termID;
    ...
    void flush(String fieldName, FieldsConsumer consumer,  final SegmentWriteState state) throws IOException 
    {
        ...
        for (int i = 0; i < numTerms; i++) 
        {
            ...
            // really TermsHashPerField should take over most of this loop, including merge sort of terms from
            // multiple threads and interacting with the TermsConsumer, only calling out to us (passing us the
            ...
            // Now termStates has numToMerge FieldMergeStates which all share the same term.  
            //Now we must interleave the docID streams.
            int docFreq = 0;
            ...
            while(true) 
            {
                ...
                // NOTE: we could check here if the docID was deleted, and skip it.  However, this is somewhat
                // dangerous because it can yield non-deterministic behavior since we may see the docID before we see
                // the term that caused it to be deleted.  This would mean some (but not all) of its postings may
                // make it into the index, which'd alter the docFreq for those terms.  We could fix this by doing two
                // passes, ie first sweep marks all del docs, and 2nd sweep does the real flush, but I suspect
