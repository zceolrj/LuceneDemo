...
class BufferedDeletesStream 
{
    ...
    // deletes applied (whose bufferedDelGen defaults to 0) will be correct:
    ...
    public synchronized long push(FrozenBufferedDeletes packet) 
    {
        ...
        if (infoStream.isEnabled("BD")) 
        {
            infoStream.message("BD", "push deletes " + packet + " delGen=" + packet.delGen() + 
                " packetCount=" + deletes.size() + " totBytesUsed=" + bytesUsed.get());
        }
        assert checkDeleteStats();
     ...
     *  each SegmentReader. */
    public synchronized ApplyDeletesResult applyDeletes(IndexWriter.ReaderPool readerPool, List<SegmentInfoPerCommit> infos) 
        throws IOException 
    {
    ...
    // Delete by query
    private static long applyQueryDeletes(Iterable<QueryAndLimit> queriesIter, ReadersAndLiveDocs rld, final SegmentReader reader) 
        throws IOException 
    {
        long delCount = 0;
    ...
    private boolean checkDeleteTerm(Term term) 
    {
        ...
        // we re-use term now in our merged iterable, but we shouldn't clone, instead copy for this assert
