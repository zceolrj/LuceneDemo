...
final class DocFieldProcessor extends DocConsumer
{
    ...
    public void flush(SegmentWriteState state) throws IOException 
    {
        ...
        storedConsumer.flush(state);//TwoStoredFieldsConsumers
        consumer.flush(childFields, state);//DocInverter
    ...
    {
        consumer.startDocument();//consumer=DocInverter
        storedConsumer.startDocument();//storedConsumer=TwoStoredFieldConsumers  first=StoredFieldProcessor second=DocValueProcessor
        ...
        // Also absorb any changes to fields we had already seen before (eg suddenly turning on norms or vectors, etc.):
        ...
        for(IndexableField field : docState.doc) 
        {
            ...
            if (fp == null) 
            {
                // FI: we need to genericize the "flags" that a field holds, and, how these flags are merged; 
                // it needs to be more "pluggable" such that if I want to have a new "thing" my Fields can do, 
                // I can easily add it
            ...
            fp.addField(field);
            storedConsumer.addField(docState.docID, field, fp.fieldInfo);//storedConsumer=TwoStoredFieldsConsumers
        }

        //If we are writing vectors then we must visit fields in sorted order so they are written in sorted order.  
        //we actually only need to sort the subset of fields that have vectors enabled; 
        //we could save [small amount of] CPU here.
        ArrayUtil.introSort(fields, 0, fieldCount, fieldsComp);
        for(int i=0;i<fieldCount;i++) 
        {
            final DocFieldProcessorPerField perField = fields[i];
            perField.consumer.processFields(perField.fields, perField.fieldCount);//perField.consumer=DocInverterPerField
        }
        ...
        if (docState.maxTermPrefix != null && docState.infoStream.isEnabled("IW")) 
        {
            docState.infoStream.message("IW", "WARNING: document contains at least one immense "
                    + "term (whose UTF8 encoding is longer than the max length " 
    ...
    void finishDocument() throws IOException 
    {
        try 
        {
            storedConsumer.finishDocument();//storedConsumer=TwoStoredFieldsConsumers
        } 
        finally 
        {
            consumer.finishDocument();//consumer=DocInverter
        }
    }
}