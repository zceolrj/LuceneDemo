 ...
 * {@link TermContext} doesn't track if the given {@link TermState} objects are
 * valid, neither if the {@link TermState} instances refer to the same terms in
...
public final class TermContext
{
	...
	/**
	 * Holds the {@link IndexReaderContext} of the top-level {@link IndexReader}
	 ...
	 */
	public TermContext(IndexReaderContext context, TermState state, int ord,
	 ...
	 * context's leaf readers and register each of the readers containing the
	 * term in the returned {@link TermContext} using the leaf reader's ordinal.
	 * <p>
			...
			if (fields != null)
			{
				...
				if (terms != null)
				{
					...
					if (termsEnum.seekExact(bytes))
					{
						...
						perReaderTermState.register(termState, ctx.ord,
	...
	/**
	 * Clears the {@link TermContext} internal state and removes all registered
	 * {@link TermState}s
	 */
	public void clear()
	{
	...
	/**
	 * Registers and associates a {@link TermState} with an leaf ordinal. The
	...
	/**
	 * Returns the accumulated term frequency of all {@link TermState} instances
	 * passed to {@link #register(TermState, int, int, long)}.
	 * 
	 * @return the accumulated term frequency of all {@link TermState} instances
	 *         passed to {@link #register(TermState, int, int, long)}.
	 */
