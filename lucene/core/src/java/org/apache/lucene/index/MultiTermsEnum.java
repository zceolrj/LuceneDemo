...
{    
    ...
    public TermsEnum reset(TermsEnumIndex[] termsEnumsIndex) throws IOException 
    {
        ...
        for(int i=0;i<termsEnumsIndex.length;i++) 
        {
            ...
            {
                // We cannot merge sub-readers that have different TermComps
                ...
                if (subTermComp != null && !subTermComp.equals(termComp)) 
                {
                  throw new IllegalStateException("sub-readers have different BytesRef.Comparators: " + 
                      subTermComp + " vs " + termComp + "; cannot merge");
                }
            }
    ...
    public boolean seekExact(BytesRef term) throws IOException 
    {
        ...
        // if at least one sub had exact match to the requested term then we found match
        return numTop > 0;
    }
    ...
    public DocsAndPositionsEnum docsAndPositions(Bits liveDocs, DocsAndPositionsEnum reuse, int flags) throws IOException 
    {
        ...
        for(int i=0;i<numTop;i++) 
        {
            ...
            assert entry.index < docsAndPositionsEnum.subDocsAndPositionsEnum.length: entry.index + 
                " vs " + docsAndPositionsEnum.subDocsAndPositionsEnum.length + "; " + subs.length;
            final DocsAndPositionsEnum subPostings = entry.terms.docsAndPositions(
                      b, docsAndPositionsEnum.subDocsAndPositionsEnum[entry.index], flags);
