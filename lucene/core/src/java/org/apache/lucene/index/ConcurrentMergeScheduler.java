...
public class ConcurrentMergeScheduler extends MergeScheduler 
{
    ...
    // Max number of merge threads allowed to be running at once.  
    // When there are more merges then this, we forcefully pause the larger ones, 
    // letting the smaller ones run, up until maxMergeCount merges at which point
    // we forcefully pause incoming threads (that presumably are the ones causing so much merging).
    private int maxThreadCount = DEFAULT_MAX_THREAD_COUNT; // 1
  
    // Max number of merges we accept before forcefully
    // throttling the incoming threads
    private int maxMergeCount = DEFAULT_MAX_MERGE_COUNT; // 2
    ...
    {
        // Only look at threads that are alive & not in the process of stopping (ie have an active merge):
    ...
    public synchronized void merge(IndexWriter writer) throws IOException 
    {
        ...
        // First, quickly run through the newly proposed merges and add any orthogonal merges 
        // (ie a merge not involving segments already pending to be merged) to the queue.  
        // If we are way behind on merging, many of these newly proposed merges will likely already be registered.
        ...
        while (true) 
        {
            ...
            {
                // This means merging has fallen too far behind: we have already created maxMergeCount threads,
                // and now there's at least one more merge pending. Note that only maxThreadCount of those created merge threads 
                // will actually be running; the rest will be paused (see updateMergeThreads).  We stall this producer
                // thread to prevent creation of new segments, until merging has caught up:
    ...
    protected class MergeThread extends Thread 
    {
        ...
        public void setThreadPriority(int pri) 
        {
            ...
            {
              // Strangely, Sun's JDK 1.5 on Linux sometimes throws NPE out of here...
            } 
            catch (SecurityException se) 
            {
              // Ignore this because we will still run fine with normal thread priority
            }
        }
    ...
    protected void handleMergeException(Throwable exc) 
    {
        try 
        {
            ...
            // so, we sleep here to avoid saturating CPU in such cases:
            Thread.sleep(250);
        } 
        catch (InterruptedException ie) 
        {
