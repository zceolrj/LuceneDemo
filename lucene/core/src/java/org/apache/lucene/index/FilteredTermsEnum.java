 ...
 * </p>
 * <p>
 * <em>Please note:</em> Consumers of this enum cannot call {@code seek()}, it
 * is forward only; it throws {@link UnsupportedOperationException} when a
...
public abstract class FilteredTermsEnum extends TermsEnum
{
	...
	protected static enum AcceptStatus
	{
		 ...
		 * {@link FilteredTermsEnum#nextSeekTerm(BytesRef)}) to the next term.
		 */
		YES_AND_SEEK,
	...
	/**
	 * Use this method to set the initial {@link BytesRef} to seek before
	 * iterating. This is a convenience method for subclasses that do not
	 * override {@link #nextSeekTerm}. If the initial seek term is {@code null}
	 * (default), the enum is empty.
	 * <P>
	 * You can only use this method, if you keep the default implementation of
	 * {@link #nextSeekTerm}.
	 */
	 ...
	 * this method will be called to eventually seek the underlying TermsEnum to
	 * a new position. On the first call, {@code currentTerm} will be
	 * {@code null}, later calls will provide the term the underlying enum is
	 * positioned at. This method returns per default only one time the initial
	 * seek term and then {@code null}, so no repositioning is ever done.
	 * <p>
	 * Override this method, if you want a more sophisticated TermsEnum, that
	 * repositions the iterator during enumeration. If this method always
	 * returns {@code null} the enum is empty.
	 * <p>
	 * <em>Please note:</em> This method should always provide a greater term
	 * than the last enumerated term, else the behaviour of this enum violates
	 * the contract for TermsEnums.
	 */
	protected BytesRef nextSeekTerm(final BytesRef currentTerm)
			throws IOException
	{
	...
	/**
	 * Returns the related attributes, the returned {@link AttributeSource} is
	 * shared with the delegate {@code TermsEnum}.
	 */
	@Override
	...
	public BytesRef next() throws IOException
	{
		...
		for (;;)
		{
			...
			if (doSeek)
			{
				...
				assert actualTerm == null || t == null
						|| getComparator().compare(t, actualTerm) > 0 : "curTerm="
