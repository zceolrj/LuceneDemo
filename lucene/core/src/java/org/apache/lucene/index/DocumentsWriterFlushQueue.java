...
class DocumentsWriterFlushQueue 
{
    ...
    void addDeletes(DocumentsWriterDeleteQueue deleteQueue) throws IOException 
    {
        synchronized (this) 
        {
            incTickets();// first inc the ticket count - freeze opens a window for #anyChanges to fail
    ...
    //purge  v.清除，（使）净化    One is to purge data; the other is to recover costs。  一个是清除数据；另一个就是收回成本
