...
{  
    ...
    class ReaderPool 
    {
        ...
        public synchronized void release(ReadersAndLiveDocs rld, boolean assertInfoLive) throws IOException 
        {
            ...
            {
                // This is the last ref to this RLD, and we're not pooling, so remove it:
            ...
            while(it.hasNext()) 
            {
                ...
                try 
                {
                    if (doSave && rld.writeLiveDocs(directory)) 
                    {
                        ...
                        // don't call IW.checkpoint because that also increments SIS.version, which we do not want to
                        // do here: it was done previously (after we invoked BDS.applyDeletes), whereas here all we
                        // did was move the state to disk:
                        checkpointNoSIS();
                    }
                } 
                ...
                // Important to remove as-we-go, not with .clear() in the end, in case we hit an exception;
                // otherwise we could over-decref if close() is called again:
                it.remove();
        
                // NOTE: it is allowed that these decRefs do not actually close the SRs; this happens when a
            ...
            for (SegmentInfoPerCommit info : infos) 
            {
                ...
                if (rld != null) 
                {
                    ...
                    if (rld.writeLiveDocs(directory)) 
                    {
                        ...
                        // which we do not want to do here: it was done previously (after we invoked BDS.applyDeletes), 
                        // whereas here all we did was move the state to disk:
                        checkpointNoSIS();
                    }
                }
            }
    ...
    public IndexWriter(Directory d, IndexWriterConfig conf) throws IOException 
    {
        ...
        try 
        {
            ...
            if (create) 
            {
                // Try to read first.  This is to allow create against an index that's currently open for searching. 
                // In this case we write the next segments_N file with no segments:
                try 
                {
            ...
            synchronized(this) 
            {
                deleter = new IndexFileDeleter(directory, config.getIndexDeletionPolicy(),
                                     segmentInfos, infoStream, this, initialIndexExists);
            }
      
            if (deleter.startingCommitDeleted) 
            {
                // Deletion policy deleted the "head" commit point. We have to mark ourself as changed 
                // so that if we are closed w/o any further changes we write a new segments_N file.
                changed();
            }
    ...
    private void closeInternal(boolean waitForMerges, boolean doFlush) throws IOException 
    {
        ...
        try 
        {
                ...
                try 
                {
                    ...
                    synchronized(this) 
                    {
                        for (;;) 
                        {
                            ...
                            {
                                // by setting the interrupted status, the next call to finishMerges will pass false,
                                // so it will not wait
                                interrupted = true;
                                if (infoStream.isEnabled("IW")) 
                                {
            ...
            synchronized(this) 
            {
                // commitInternal calls ReaderPool.commit, which writes any pending liveDocs from ReaderPool, 
                // so it's safe to drop all readers now:
            ...
            assert oldWriter.perThreadPool.numDeactivatedThreadStates() == oldWriter.perThreadPool.getMaxThreadStates() : "" +  
                              oldWriter.perThreadPool.numDeactivatedThreadStates() + " " +  oldWriter.perThreadPool.getMaxThreadStates();
        } 
        catch (OutOfMemoryError oom) 
        {
     ...
     * @lucene.experimental
     */
    public void updateDocuments(Term delTerm, Iterable<? extends Iterable<? extends IndexableField>> docs, Analyzer analyzer) 
        throws IOException 
    {
        ensureOpen();
        try 
    ...
    public synchronized boolean tryDeleteDocument(IndexReader readerIn, int docID) throws IOException 
    {
        ...
        // seriously wrong w/ the index, so it should be a minor cost:
            ...
            if (rld != null) 
            {
              synchronized(bufferedDeletesStream) 
              {
                  ...
                  if (rld.delete(docID)) 
                  {
                      ...
                      // Must bump changeCount so if no other changes happened, we still commit this change:
                      changed();
                  }
    ...
    final String newSegmentName() 
    {
        ...
        synchronized(segmentInfos) 
        {
            ...
            // Otherwise we could close, re-open and re-return the same segment name that was previously 
            // returned which can cause problems at least with ConcurrentMergeScheduler.
            changeCount++;
    ...
    */
    ...
    private synchronized void updatePendingMerges(MergeTrigger trigger, int maxNumSegments) throws IOException 
    {
    ...
    private synchronized void finishMerges(boolean waitForMerges) 
    {
        ...
        {
            // waitForMerges() will ensure any running addIndexes finishes. It's fine 
            // if a new one attempts to start because from our caller above the call 
            // will see that we are in the process of closing, and will throw an AlreadyClosedException.
            waitForMerges();
        }
    }
            ...
            try 
            {
                for (Directory dir : dirs) 
                {
                    ...
                    for (SegmentInfoPerCommit info : sis) 
                    {
                        ...
                        if (infoStream.isEnabled("IW")) 
                        {
                            infoStream.message("IW", "addIndexes: process segment origName=" + info.info.name + 
    ...
    public void addIndexes(IndexReader... readers) throws IOException 
    {
        ...
        try 
        {
            ...
            // and 2) .si reflects useCompoundFile=true change above:
            success = false;
            try 
            {
    ...
    private SegmentInfoPerCommit copySegmentAsIs(SegmentInfoPerCommit info, String segName,
                Map<String, String> dsNames, Set<String> dsFilesCopied, IOContext context, Set<String> copiedFiles)
        throws IOException 
    {
    ...
    private void prepareCommitInternal() throws IOException 
    {
        synchronized(commitLock) 
        {
            ...
            // clone & incRef the flushed SegmentInfos inside the sync block:
                ...
                synchronized (fullFlushLock) 
                {
                    ...
                    try 
                    {
                        ...
                        synchronized(this) 
                        {
                            ...
                            // Must clone the segmentInfos while we still hold fullFlushLock and  while sync'd 
                            // so that no partial changes (eg a delete w/o corresponding add from an updateDocument) 
                            // can sneak into the commit point:
                            toCommit = segmentInfos.clone();
              
                            pendingCommitChangeCount = changeCount;
              
                            // This protects the segmentInfos we are now going to commit.  This is important in case,
                            // eg, while we are trying to sync all referenced files, a merge completes which
                            // would otherwise have removed the files we are now syncing.    
    ...
    {
        // NOTE: this method cannot be sync'd because maybeMerge() in turn calls mergeScheduler.
        // merge which in turn can take a long time to run and we don't want to hold the lock for that.  
        // In the case of ConcurrentMergeScheduler this can lead to deadlock
    ...
    final synchronized void applyAllDeletes() throws IOException 
    {
        ...
        if (!keepFullyDeletedSegments && result.allDeleted != null) 
        {
            ...
            {
                // If a merge has already registered for this segment, we leave it in the readerPool; 
                // the merge will skip merging it and will then drop it once it's done:
        ...
        for(SegmentInfoPerCommit info : merge.segments) 
        {
            if (!segmentInfos.contains(info)) 
            {
                throw new MergePolicy.MergeException("MergePolicy selected a segment (" 
                    + info.info.name + ") that is not in the current index " + segString(), directory);
            }
        }
    }
        ...
        for(int i=0; i < sourceSegments.size(); i++) 
        {
            ...
            if (prevLiveDocs != null) 
            {
                ...
                if (currentLiveDocs != prevLiveDocs) 
                {
                    ...
                    // since we started the merge, so we must merge them:
    ...
    synchronized private boolean commitMerge(MergePolicy.OneMerge merge, MergeState mergeState) throws IOException 
    {
        ...
        if (infoStream.isEnabled("IW")) 
        {
            if (allDeleted) 
            {
                infoStream.message("IW", "merged segment " + merge.info + " is 100% deleted" +  
        ...
        // merge segment, in which case we need to abort the merge:
        ...
        try 
        {
            ...
            // able to delete the held-open files from the merge readers:
    ...
    final synchronized boolean registerMerge(MergePolicy.OneMerge merge) throws IOException 
    {
        ...
        for(SegmentInfoPerCommit info : merge.segments) 
        {
            if (mergingSegments.contains(info)) 
            {
                if (infoStream.isEnabled("IW")) 
                {
                    infoStream.message("IW", "reject merge " + segString(merge.segments) + 
                        ": segment " + segString(info) + " is already marked for merge");
                }
                return false;
            }
            if (!segmentInfos.contains(info)) 
            {
                ...
                {
                    infoStream.message("IW", "reject merge " + segString(merge.segments) + 
                        ": segment " + segString(info) + " does not exist in live infos");
                }
                return false;
            }
        ...
        if (infoStream.isEnabled("IW")) 
        {
            infoStream.message("IW", "add merge to pendingMerges: " + segString(merge.segments) + 
    ...
    synchronized private void _mergeInit(MergePolicy.OneMerge merge) throws IOException 
    {
        ...
        // ConcurrentMergePolicy we keep deterministic segment names.
    ...
    private int mergeMiddle(MergePolicy.OneMerge merge) throws IOException 
    {
        ...
        try 
        {
            ...
            while(segUpto < sourceSegments.size()) 
            {
                ...
                synchronized(this) 
                {
                    ...
                    if (infoStream.isEnabled("IW")) 
                    {
                        if (rld.getPendingDeleteCount() != 0) 
                        {
                            infoStream.message("IW", "seg=" + segString(info) + 
                                " delCount=" + info.getDelCount() + " pendingDelCount=" + rld.getPendingDeleteCount());
                        } 
                ...
                assert delCount <= info.info.getDocCount(): "delCount=" + delCount + 
                    " info.docCount=" + info.info.getDocCount() + " rld.pendingDeleteCount=" + 
                    rld.getPendingDeleteCount() + " info.getDelCount()=" + info.getDelCount();
                segUpto++;
            }
            ...
            if (infoStream.isEnabled("IW")) 
            {
                ...
                {
                    infoStream.message("IW", "merge codec=" + codec + " docCount=" + merge.info.info.getDocCount() + 
                                      "; merged segment has " +
            ...
            if (infoStream.isEnabled("IW")) 
            {
                infoStream.message("IW", String.format(Locale.ROOT, "merged segment size=%.3f MB vs estimate=%.3f MB", 
                    merge.info.sizeInBytes()/1024./1024., merge.estimatedMergeBytes/1024/1024.));
            }
    ...
    private boolean filesExist(SegmentInfos toSync) throws IOException 
    {
        ...
        for(final String fileName: files) 
        {
            ...
            // file referenced by the current head segmentInfos:
        ...
        try 
        {
            ...
            try 
            {
                ...
                synchronized(this) 
                {
                    ...
                    // (this method unwinds everything it did on an exception)
            ...
            finally 
            {
                synchronized(this) 
                {
                    // Have our master segmentInfos record the generations we just prepared. 
                    // We do this on error or success so we don't double-write a segments_N file.
    ...
    synchronized boolean nrtIsCurrent(SegmentInfos infos) 
    {
        ...
        if (infoStream.isEnabled("IW")) 
        {
            infoStream.message("IW", "nrtIsCurrent: infoVersion matches: " + (infos.version == segmentInfos.version) + 
                "; DW changes: " + docWriter.anyChanges() + "; BD changes: "+ bufferedDeletesStream.any());
        }
    ...
    static final Collection<String> createCompoundFile(InfoStream infoStream, Directory directory, 
        CheckAbort checkAbort, final SegmentInfo info, IOContext context) throws IOException 
    {
