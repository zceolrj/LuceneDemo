...
final class DocInverterPerField extends DocFieldConsumerPerField 
{
    ...
    public void processFields(final IndexableField[] fields, final int count) throws IOException 
    {
        ...
        final boolean doInvert = consumer.start(fields, count);//consumer=TermsHashPerField
        ...
        for(int i=0;i<count;i++) 
        {
            ...
            // FI: this should be "genericized" to querying consumer if it wants to see this particular field tokenized.
            if (fieldType.indexed() && doInvert) 
            {
                ...
                // after we fix analyzers, also check if termVectorOffsets will be indexed.
                ...
                try 
                {
                    ...
                    if (hasMoreTokens) 
                    {
                        consumer.start(field);//consumer=TermsHashPerField
                        ...
                        do 
                        {
                            ...
                            if (posIncr < 0) 
                            {
                                throw new IllegalArgumentException("position increment must be >=0 (got " + posIncr + ") for field '" 
                                                  + field.name() + "'");
                            }
                            ...
                            try 
                            {
                                ...
                                // flush, on the likelihood that the internal state of the consumer is now
                                // corrupt and should not be flushed to a new segment:
                                consumer.add();//consumer=TermsHashPerField
                                success = true;
                            } 
                            finally 
                            {
                    ...
                    stream.end();
                    // maybe add some safety? then again, its already checked 
        ...
        consumer.finish();//consumer=TermsHashPerField
        endConsumer.finish();//endConsumer=NormsConsumerPerField
    }
