...
final class TermVectorsConsumerPerField extends TermsHashConsumerPerField 
{
    ...
    boolean start(IndexableField[] fields, int count) 
    {
        ...
        for(int i=0;i<count;i++) 
        {
            ...
            if (field.fieldType().indexed()) 
            {
                if (field.fieldType().storeTermVectors()) 
                {
                    ...
                    {
                        // move this check somewhere else, and impl the other missing ones
                        throw new IllegalArgumentException("cannot index term vector payloads without term vector positions (field=\"" 
                                          + field.name() + "\")");
                    }
                } 
                else 
                {
                    ...
                    {
                        throw new IllegalArgumentException("cannot index term vector offsets when term vectors are not indexed (field=\"" 
                                          + field.name() + "\")");
                    }
                    if (field.fieldType().storeTermVectorPositions()) 
                    {
                        throw new IllegalArgumentException("cannot index term vector positions when term vectors are not indexed (field=\"" 
                                          + field.name() + "\")");
                    }
                    if (field.fieldType().storeTermVectorPayloads()) 
                    {
                        throw new IllegalArgumentException("cannot index term vector payloads when term vectors are not indexed (field=\"" 
                                          + field.name() + "\")");
                    }
                }
            } 
            else 
            {
                if (field.fieldType().storeTermVectors()) 
                {
                    throw new IllegalArgumentException("cannot index term vectors when field is not indexed (field=\"" 
                                      + field.name() + "\")");
                }
                if (field.fieldType().storeTermVectorOffsets()) 
                {
                    throw new IllegalArgumentException("cannot index term vector offsets when field is not indexed (field=\"" 
                                      + field.name() + "\")");
                }
                if (field.fieldType().storeTermVectorPositions()) 
                {
                    throw new IllegalArgumentException("cannot index term vector positions when field is not indexed (field=\"" 
                                      + field.name() + "\")");
                }
                if (field.fieldType().storeTermVectorPayloads()) 
                {
                    throw new IllegalArgumentException("cannot index term vector payloads when field is not indexed (field=\"" 
                                      + field.name() + "\")");
                }
            }
        }
        ...
        if (doVectors) 
        {
            ...
            {
                // Only necessary if previous doc hit a non-aborting exception while writing vectors in this field:
                termsHashPerField.reset();
            }
        }
    
        // only if needed for performance perThread.postingsCount = 0;
    ...
    void finishDocument() throws IOException 
    {
        ...
        // This is called once, after inverting all occurrences of a given field in the doc.  
        // At this point we flush our hash into the DocWriter.
