 ...
 * levels of exponentially increasing size, where each level has fewer segments
 * than the value of the merge factor. Whenever extra segments (beyond the merge
 * factor upper bound) are encountered, all segments within the level are
 * merged. You can get or set the merge factor using {@link #getMergeFactor()}
 * and {@link #setMergeFactor(int)} respectively.
 * </p>
 *
 * <p>
...
public abstract class LogMergePolicy extends MergePolicy
{
	 ...
	 * computed by taking the max segment log size, minus LEVEL_LOG_SPAN, and
	...
	/**
	 * Any segments whose size is smaller than this value will be rounded up to
	 * this value. This ensures that tiny segments are aggressively merged.
	 */
	...
	/**
	 * If the size of a segment exceeds this value then it will never be merged
	 * during {@link IndexWriter#forceMerge}.
	 */
	protected long maxMergeSizeForForcedMerge = Long.MAX_VALUE;

	/**
	 * If a segment has more than this many documents then it will never be
	 * merged.
	 */
	protected int maxMergeDocs = DEFAULT_MAX_MERGE_DOCS;

	/**
	 * If true, we pro-rate a segment's size by the percentage of non-deleted
	 * documents.
	 */
	...
	/**
	 * Returns true if {@code LMP} is enabled in {@link IndexWriter}'s
	 * {@code infoStream}.
	 */
	...
	/**
	 * Print a debug message to {@link IndexWriter}'s {@code infoStream}.
	 */
	 ...
	 * smaller values, less RAM is used while indexing, and searches are faster,
	 * but indexing speed is slower. With larger values, more RAM is used during
	 * indexing, and while searches is slower, indexing is faster. Thus larger
	 * values (> 10) are best for batch index creation, and smaller values (<
	...
	/**
	 * Sets whether the segment size should be calibrated by the number of
	 * deletes when choosing segments for merge.
	 */
	...
	/**
	 * Returns true if the segment size should be calibrated by the number of
	 * deletes when choosing segments for merge.
	 */
	 ...
	 * {@link SegmentInfoPerCommit}, pro-rated by percentage of non-deleted
	 * documents if {@link #setCalibrateSizeByDeletes} is set.
	 */
	...
	/**
	 * Return the byte size of the provided {@link SegmentInfoPerCommit},
	 * pro-rated by percentage of non-deleted documents if
	 * {@link #setCalibrateSizeByDeletes} is set.
	 */
	...
	/**
	 * Returns true if the number of segments eligible for merging is less than
	 * or equal to the specified {@code maxNumSegments}.
	 */
	 ...
	 * size or max merge docs into consideration. This method attempts to
	 * respect the {@code maxNumSegments} parameter, however it might be, due to
	 * size constraints, that more than that number of segments will remain in
	 * the index. Also, this method does not guarantee that exactly
	 * {@code maxNumSegments} will remain, but &lt;= that number.
	 */
			...
			int maxNumSegments, int last) throws IOException
	{
		...
		while (start >= 0)
		{
					...
					|| sizeDocs(info) > maxMergeDocs)
			{
				...
				// need to skip that segment + add a merge for the 'right'
				// segments,
				// unless there is only 1 which is merged.
				if (last - start - 1 > 1
						|| (start != last - 1 && !isMerged(infos.info(start + 1))))
				{
			...
			{
				// mergeFactor eligible segments were found, add them as a
				// merge.
	 ...
	 * parameter, and guaranteed that exactly that number of segments will
	 * remain in the index.
	 */
	...
	/**
	 * Returns the merges necessary to merge the index down to a specified
	 * number of segments. This respects the {@link #maxMergeSizeForForcedMerge}
	 * setting. By default, and assuming {@code maxNumSegments=1}, only one
	 * segment will be left in the index, where that segment has no deletions
	 * pending nor separate norms, and it is in compound file format if the
	 * current useCompoundFile setting is true. This method returns multiple
	 * merges (mergeFactor at a time) so the {@link MergeScheduler} in use may
	...
	/**
	 * Finds merges necessary to force-merge all deletes from the index. We
	 * simply merge adjacent segments that have deletes, up to mergeFactor at a
	 * time.
	 */
	@Override
	...
	private static class SegmentInfoAndLevel implements Comparable<SegmentInfoAndLevel>
	{
		...
		public SegmentInfoAndLevel(SegmentInfoPerCommit info, float level,
				int index)
		{
			this.info = info;
			this.level = level;
			...
			SegmentInfos infos) throws IOException
	{
		...
		for (int i = 0; i < numSegments; i++)
		{
			...
			if (verbose())
			{
				...
				message("seg="
						+ writer.get().segString(info)
						+ " level="
						+ infoLevel.level
						+ " size="
						+ String.format(Locale.ROOT, "%.3f MB",
								segBytes / 1024 / 1024.) + extra);
			}
		}
	 ...
	 *
	 * <p>
	 * The default merge policy ({@link LogByteSizeMergePolicy}) also allows you
	 * to set this limit by net size (in MB) of the segment, using
	 * {@link LogByteSizeMergePolicy#setMaxMergeMB}.
	 * </p>
	 */
