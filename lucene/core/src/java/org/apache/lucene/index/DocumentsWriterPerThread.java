...
class DocumentsWriterPerThread 
{
    ...
    {
        @Override
        DocConsumer getChain(DocumentsWriterPerThread documentsWriterPerThread) //DocFieldProcessor implements DocConsumer
        {
            /*
    ...
    public DocumentsWriterPerThread(String segmentName, Directory directory, LiveIndexWriterConfig indexWriterConfig, 
        InfoStream infoStream, DocumentsWriterDeleteQueue deleteQueue, FieldInfos.Builder fieldInfos) 
    {
    ...
    public void updateDocument(Iterable<? extends IndexableField> doc, Analyzer analyzer, Term delTerm) 
        throws IOException 
    {
        ...
        try 
        {
            try 
            {
                consumer.processDocument(fieldInfos); //consumer=DocFieldProcessor
            } 
            finally 
            {
                docState.clear();
        ...
        try 
        {
            consumer.finishDocument();//consumer=DocFieldProcessor
            success = true;
        } 
        finally 
        {
    ...
    public int updateDocuments(Iterable<? extends Iterable<? extends IndexableField>> docs, Analyzer analyzer, Term delTerm) throws IOException 
    {
        ...
        try 
        {
            ...
            // Apply delTerm only after all indexing has succeeded, 
            // but apply it only to docs prior to when this batch started:
            if (delTerm != null) 
            {
    ...
    void deleteDocID(int docIDUpto) 
    {
      ...
      // NOTE: we do not trigger flush here.  This is potentially a RAM leak, 
      // if you have an app that tries to add docs but every single doc always hits a non-aborting exception.  
      // Allowing a flush here gets very messy because we are only invoked when handling exceptions so to do this properly, 
      // while handling an exception we'd have to go off and flush new deletes
    ...
    FlushedSegment flush() throws IOException 
    {
        ...
        // Apply delete-by-docID now (delete-byDocID only happens when an exception is hit 
    ...
    void sealFlushedSegment(FlushedSegment flushedSegment) throws IOException 
    {
        ...
        try 
        {
            ...
            // Have codec write SegmentInfo.  Must do this after creating CFS so that 1) .si isn't slurped into CFS,
            ...
            // ideally we would freeze newSegment here!!
            // because any changes after writing the .si will be lost... 
      
            // Must write deleted docs after the CFS so we don't slurp the del file into CFS:
            if (flushedSegment.liveDocs != null) 
            {
                ...
                // we should prune the segment if it's 100% deleted... but merge will also catch it.
        
                // in the NRT case it'd be better to hand this del vector over to the shortly-to-be-opened 
                // SegmentReader and let it carry the changes; there's no reason to use filesystem as intermediary here.
