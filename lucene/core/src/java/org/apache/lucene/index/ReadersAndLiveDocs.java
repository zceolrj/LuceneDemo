...
// searching or merging), plus pending deletes, for a given segment
class ReadersAndLiveDocs 
{
    // Not final because we replace (clone) when we need to change it and it's been shared:
    ...
    // it's sometimes wasteful that we hold open two
    ...
    public synchronized boolean delete(int docID) 
    {
        ...
        assert docID >= 0 && docID < liveDocs.length() : "out of bounds: docid=" + docID + 
            " liveDocsLength=" + liveDocs.length() + " seg=" + info.info.name + " docCount=" + info.info.getDocCount();
        assert !shared;
    ...
    {
        // can we somehow use IOUtils here...?  problem is we are calling .decRef not .close)...
        try 
        {
            if (reader != null) 
            {
    ...
    public synchronized SegmentReader getReadOnlyClone(IOContext context) throws IOException 
    {
        ...
        if (liveDocs != null) 
        {
            return new SegmentReader(reader.getSegmentInfo(), reader.core, liveDocs, 
    ...
    public synchronized boolean writeLiveDocs(Directory dir) throws IOException 
    {
        ...
        if (pendingDeleteCount != 0) 
        {
            ...
            // .tmp & renaming it) because the file is not live until segments file is written:
            boolean success = false;
            try 
            {
                info.info.getCodec().liveDocsFormat().writeLiveDocs((MutableBits)liveDocs, trackingDir, 
                    info, pendingDeleteCount, IOContext.DEFAULT);
                success = true;
            } 
            finally 
            {
