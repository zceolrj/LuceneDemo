...
final class TermsHashPerField extends InvertedDocConsumerPerField 
{
        ...
        final TermsHash nextTermsHash, final FieldInfo fieldInfo) 
    {
        ...
        if (nextTermsHash != null)//nextTermsHash=TermsHash
        {
    ...
    void start(IndexableField f) 
    {
        ...
        consumer.start(f);//consumer=FreqProxTermsWriterPerField
        if (nextPerField != null) 
        {
            nextPerField.start(f);//nextPerField=TermsHashPerField
        }
    }
    ...
    {
        doCall = consumer.start(fields, count);//consumer=FreqProxTermsWriterPerField
        bytesHash.reinit();
        if (nextPerField != null) 
        {
            doNextCall = nextPerField.start(fields, count);//nextPerField=TermsHashPerField
        }
    ...
    public void add(int textStart) throws IOException 
    {
        ...
        {      
            // First time we are seeing this token since we last flushed the hash.   Init stream slices
    ...
    void add() throws IOException 
    {
        ...
        catch (MaxBytesLengthExceededException e) 
        {
            ...
            // Just skip this term, to remain as robust as possible during indexing.  
            // A TokenFilter can be inserted into the analyzer chain if other behavior is wanted 
            // (pruning the term to a prefix, throwing an exception, etc).
    ...
    {
        consumer.finish();//consumer=FreqProxTermsWriterPerField
        if (nextPerField != null)
        {
            nextPerField.finish();//nextPerField=TermsHashPerField
        }
    }
