...
final class DocumentsWriterFlushControl
{
	...
	final DocumentsWriterStallControl stallControl;// stall v.失速、拖延、暂缓
	...
	private long stallLimitBytes()
	{
		...
		return maxRamMB != IndexWriterConfig.DISABLE_AUTO_FLUSH ? (long) (2 * (maxRamMB * 1024 * 1024))
				: Long.MAX_VALUE;
	}
	...
	private boolean assertMemory()
	{
		...
		if (maxRamMB != IndexWriterConfig.DISABLE_AUTO_FLUSH)
		{
			...
			// 2 * ramBufferBytes -> before we stall we need to cross the 2xRAM
			// Buffer border this is still a valid limit
			// (numPending + numFlushingDWPT() + numBlockedFlushes()) *
			// peakDelta) -> those are the total number of DWPT that are not
			// active but not yet fully fluhsed
			// all of them could theoretically be taken out of the loop once
			// they crossed the RAM buffer and the last document was the peak
			// delta
			// (numDocsSinceStalled * peakDelta) -> at any given time there
			// could be n threads in flight that crossed the stall control
			// before we reached the limit and each of them could hold a peak
			// document
			final long expected = (2 * (ramBufferBytes))
			...
			if (peakDelta < (ramBufferBytes >> 1))
			{
				 ...
				 * asserting the memory which would in turn fail. To prevent
				 * this we only assert if the the largest document seen is
				 * smaller than the 1/2 of the maxRamBufferMB
				 */
				assert ram <= expected : "actual mem: " + ram
						+ " byte, expected mem: " + expected
						+ " byte, flush mem: " + flushBytes + ", active mem: "
						+ activeBytes + ", pending DWPT: " + numPending
						+ ", flushing DWPT: " + numFlushingDWPT()
						+ ", blocked DWPT: " + numBlockedFlushes()
	...
	synchronized DocumentsWriterPerThread doAfterDocument(
			ThreadState perThread, boolean isUpdate)
	{
		try
		{
			...
			if (!perThread.flushPending)
			{
						...
						&& perThread.bytesUsed > hardMaxBytesPerDWPT)
				{
					...
					// limit. This
					// is super important since we can not address more than
					// 2048 MB per DWPT
	...
	private boolean assertNumDocsSinceStalled(boolean stalled)
	{
		 ...
		 * the stall control check before we actually stalled. see
		 * #assertMemory()
		 */
		if (stalled)
		{
	...
	private final boolean updateStallState()
	{
		...
		/*
		 * we block indexing threads if net byte grows due to slow flushes yet,
		 * for small ram buffers and large documents we can easily reach the
		 * limit without any ongoing flushes. we need to ensure that we don't
		 * stall/block if an ongoing or pending flush can not free up enough
		 * memory to release the stall lock.
		 */
		final boolean stall = ((activeBytes + flushBytes) > limit)
				&& (activeBytes < limit) && !closed;
	 ...
	 * {@link ThreadState} must have indexed at least on Document and must not
	 * be already pending.
	 */
	public synchronized void setFlushPending(ThreadState perThread)
	{
		...
		if (perThread.dwpt.getNumDocsInRAM() > 0)
		{
			...
			assert assertMemory();
		} // don't assert on numDocs since we could hit an abort excp. while
			...
			if (perThread.tryLock())
			{
				try
				{
					if (perThread.isInitialized())
					{
						...
						final DocumentsWriterPerThread dwpt;
						final long bytes = perThread.bytesUsed; // do that
																// before
																// replace!
						dwpt = perThreadPool.reset(perThread, closed);
	...
	public int getNumGlobalTermDeletes()
	{
		return documentsWriter.deleteQueue.numGlobalTermDeletes()
				+ bufferedDeletesStream.numTerms();
	}

	public long getDeleteBytesUsed()
	{
		return documentsWriter.deleteQueue.bytesUsed()
	...
	ThreadState obtainAndLock()
	{
		final ThreadState perThread = perThreadPool.getAndLock(
				Thread.currentThread(), documentsWriter);
		...
		try
		{
			...
			{
				// There is a flush-all in process and this DWPT is now stale --
				// enroll it for flush and try for another DWPT:
				addFlushableState(perThread);
			}
			success = true;
			// simply return the ThreadState even in a flush all case sine we
	...
	void markForFullFlush()
	{
		...
		synchronized (this)
		{
			...
			assert fullFlushBuffer.isEmpty() : "full flush buffer should be empty: "
					+ fullFlushBuffer;
			fullFlush = true;
			flushingQueue = documentsWriter.deleteQueue;
			// Set a new delete queue - all subsequent DWPT will use this queue
			// until we do another full flush
			DocumentsWriterDeleteQueue newQueue = new DocumentsWriterDeleteQueue(
					flushingQueue.generation + 1);
		...
		synchronized (this)
		{
			 ...
			 * pending and moved to blocked are moved over to the flushQueue.
			 * There is a chance that this happens since we marking DWPT for
			 * full flush without blocking indexing.
			 */
	...
	private boolean assertActiveDeleteQueue(DocumentsWriterDeleteQueue queue)
	{
		...
		for (int i = 0; i < limit; i++)
		{
			...
			try
			{
				assert !next.isInitialized() || next.dwpt.deleteQueue == queue : "isInitialized: "
						+ next.isInitialized()
						+ " numDocs: "
						+ (next.isInitialized() ? next.dwpt.getNumDocsInRAM()
								: 0);
			}
			finally
			{
	 ...
	 * given flush queue.
	 */
	private void pruneBlockedQueue(
			final DocumentsWriterDeleteQueue flushingQueue)
	{
		...
		while (iterator.hasNext())
		{
			...
			if (blockedFlush.dwpt.deleteQueue == flushingQueue)
			{
				...
				flushingWriters.put(blockedFlush.dwpt,
						Long.valueOf(blockedFlush.bytes));
				// don't decr pending here - its already done when DWPT is
				// blocked
	...
	synchronized void abortPendingFlushes(Set<String> newFiles)
	{
		try
		{
			for (DocumentsWriterPerThread dwpt : flushQueue)
			{
				try
				{
					documentsWriter.subtractFlushedNumDocs(dwpt
							.getNumDocsInRAM());
					dwpt.abort(newFiles);
				}
				catch (Throwable ex)
			...
			for (BlockedFlush blockedFlush : blockedFlushes)
			{
				try
				{
					flushingWriters.put(blockedFlush.dwpt,
							Long.valueOf(blockedFlush.bytes));
					documentsWriter.subtractFlushedNumDocs(blockedFlush.dwpt
							.getNumDocsInRAM());
	...
	void waitIfStalled()
	{
		if (infoStream.isEnabled("DWFC"))
		{
			infoStream.message("DWFC", "waitIfStalled: numFlushesPending: "
					+ flushQueue.size() + " netBytes: " + netBytes()
					+ " flushBytes: " + flushBytes() + " fullFlush: "
					+ fullFlush);
		}
