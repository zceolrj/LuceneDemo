...
class AutomatonTermsEnum extends FilteredTermsEnum
{
	...
	// in this case it is faster to drive the query based on the terms
	// dictionary.
	...
	/**
	 * Construct an enumerator based upon an automaton, enumerating the
	 * specified field, working on a supplied TermsEnum
	 * <p>
	 * 
	...
	/**
	 * Returns true if the term matches the automaton. Also stashes away the
	 * term to assist with smart enumeration.
	 */
	@Override
	protected AcceptStatus accept(final BytesRef term)
	{
				...
				|| StringHelper.endsWith(term, commonSuffixRef))
		{
			if (runAutomaton.run(term.bytes, term.offset, term.length))
				...
				return (linear && termComp.compare(term, linearUpperBound) < 0) ? AcceptStatus.NO
	...
	/**
	 * Sets the enum to operate in linear fashion, as we have found a looping
	 * transition at position: we set an upper bound and act like a
	 ...
	 * that will not put the machine into a reject state. If such a string does
	...
	private boolean nextString()
	{
		...
		while (true)
		{
			...
			for (state = states[pos]; pos < seekBytesRef.length; pos++)
			{
				...
				int nextState = runAutomaton.step(state,
						seekBytesRef.bytes[pos] & 0xff);
				if (nextState == -1)
					break;
			...
			// take the useful portion, and the last non-reject state, and
			// attempt to
			...
			{ /* no more solutions exist from this useful portion, backtrack */
				if ((pos = backtrack(pos)) < 0) /* no more solutions at all */
					return false;
				final int newState = runAutomaton.step(states[pos],
						seekBytesRef.bytes[pos] & 0xff);
				if (newState >= 0 && runAutomaton.isAccept(newState))
					/* String is good to go as-is */
					return true;
				/* else advance further */
				// paranoia? if we backtrack thru an infinite DFA, the loop
	 ...
	 * 
	 * If this cannot satisfy the machine, returns false. This method will walk
	 * the minimal path, in lexicographic order, as long as possible.
	 * 
	...
	private boolean nextString(int state, int position)
	{
		...
		if (position < seekBytesRef.length)
		{
			...
			// then by definition it puts us in a reject state, and therefore
			// this
		...
		for (int i = 0; i < transitions.length; i++)
		{
			...
			if (transition.getMax() >= c)
			{
				...
				// append either the next sequential char, or the minimum
				// transition
				 ...
				 * lexicographic order. if a loop or accept state is
				 * encountered, stop.
				 */
				while (visited[state] != curGen
						&& !runAutomaton.isAccept(state))
				{
					visited[state] = curGen;
					/*
					 * Note: we work with a DFA with no transitions to dead
					 * states. so the below is ok, if it is not an accept state,
					...
					seekBytesRef.grow(seekBytesRef.length + 1);
					seekBytesRef.length++;
					seekBytesRef.bytes[seekBytesRef.length - 1] = (byte) transition
							.getMin();
	...
	/**
	 * Attempts to backtrack thru the string after encountering a dead end at
	 * some given position. Returns false if no more possible strings can match.
	 * 
	 * @param position
