...
public class BooleanQuery extends Query implements Iterable<BooleanClause>
{
	 ...
	 * 
	 */
		private static final long serialVersionUID = 5840009836150077432L;
	 ...
	 * {@link FuzzyQuery}.
	 *
	 * @param disableCoord disables {@link Similarity#coord(int,int)} in
	 *            scoring.
	 */
	...
	protected class BooleanWeight extends Weight
	{
				...
				throws IOException
		{
			...
			for (Iterator<Weight> wIter = weights.iterator(); wIter.hasNext();)
			{
				...
				if (w.scorer(context, true, true, context.reader()
						.getLiveDocs()) == null)
				{
					if (c.isRequired())
					{
				...
				throws IOException
		{
			...
			// TODO: (LUCENE-4872) in some cases BooleanScorer may be faster for
			// minNrShouldMatch
			...
			{
				float coord = disableCoord ? 1.0f : coord(required.size(),
						maxCoord);
				return new ConjunctionScorer(this,
						required.toArray(new Scorer[required.size()]), coord);
			}

			// simple disjunction
			if (required.size() == 0 && prohibited.size() == 0
					&& minNrShouldMatch <= 1 && optional.size() > 1)
			{
				...
				return new DisjunctionSumScorer(this,
						optional.toArray(new Scorer[optional.size()]), coord);
			}
		...
		{ // optimize 1-clause queries
			...
			{ // just return clause
				...
				{ // incorporate boost
					...
					// Since the BooleanQuery only has 1 clause, the
					// BooleanQuery will be
					// written out. Therefore the rewritten Query's boost must
					// incorporate both
					// the clause's boost, and the boost of the BooleanQuery
					// itself
		...
		for (int i = 0; i < clauses.size(); i++)
		{
			...
			{ // clause rewrote: must clone
				if (clone == null)
				{
					...
					// initialize
					// it if a rewritten clause differs from the original clause
					// (and hasn't been
