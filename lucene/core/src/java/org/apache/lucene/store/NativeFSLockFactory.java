 ...
 * 
 * <p>
 * Note that, unlike {@link SimpleFSLockFactory}, the existence of leftover lock
 * files in the filesystem on exiting the JVM is fine because the OS will free
 * the locks held against these files even though the files still remain.
 * </p>
 *
 * <p>
...
public class NativeFSLockFactory extends FSLockFactory
{
	 ...
	 * When you pass this factory to a {@link FSDirectory} subclass, the lock
	 * directory is automatically set to the directory itself. Be sure to create
	 * one instance for each directory your create!
	 */
	...
	public void clearLock(String lockName) throws IOException
	{
		...
		if (lockDir.exists())
		{
			...
			// Try to release the lock first - if it's held by another process,
			// this
			// method should not silently fail.
			// NOTE: makeLock fixes the lock name by prefixing it w/ lockPrefix.
			// Therefore it should be called before the code block next which
			// prefixes
			// the given name.
			...
			// As mentioned above, we don't care if the deletion of the file
			// failed.
...
class NativeFSLock extends Lock
{
	 ...
	 * of a FileChannel (per JVM) for all locking against a given file (locks
	 * are tracked per FileChannel instance in Java 1.4/1.5). Even using the
	 * same FileChannel instance is not completely thread-safe with Java 1.4/1.5
	 * though. To work around this, we have a single (static) HashSet that
	 * contains the file paths of all currently locked locks. This protects
	 * against possible cases where different Directory instances in one JVM
	 * (each with their own NativeFSLockFactory instance) have set the same lock
	 * dir and lock prefix. However, this will not work when LockFactorys are
	 * created by different classloaders (eg multiple webapps).
	 * 
	 * Java 1.6 tracks system wide locks in a thread safe manner (same
	 * FileChannel instance or not), so we may want to change this when Lucene
	 * moves to Java 1.6.
	 */
	...
	public synchronized boolean obtain() throws IOException
	{
		...
		{
			// NoSuchDirectoryException instead?
	...
	public synchronized void release() throws IOException
	{
		if (lockExists())
		{
			...
			// because it's held up by another process (e.g. AntiVirus).
			// NativeFSLock
		...
		{
			// if we don't hold the lock, and somebody still called release(),
			// for
			// example as a result of calling IndexWriter.unlock(), we should
			// attempt
			// to obtain the lock and release it. If the obtain fails, it means
			// the
			// lock cannot be released, and we should throw a proper exception
			// rather
	...
	{
		// The test for is isLocked is not directly possible with native file
		// locks:
		...
		// Look if lock file is present; if not, there can definitely be no
		// lock!
		if (!path.exists())
			return false;
