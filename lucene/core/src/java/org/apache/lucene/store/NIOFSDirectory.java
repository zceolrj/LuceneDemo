...
public class NIOFSDirectory extends FSDirectory 
{
    ...
    protected static class NIOFSIndexInput extends FSIndexInput 
    {
      ...
      public NIOFSIndexInput(String sliceDescription, File path, RandomAccessFile file, 
          FileChannel fc, long off, long length, int bufferSize) 
      {
          super("NIOFSIndexInput(" + sliceDescription + " in path=\"" + path + "\" slice=" + 
              off + ":" + (off+length) + ")", file, off, length, bufferSize);
          channel = fc;
          isClone = true;
      }
      ...
      protected void readInternal(byte[] b, int offset, int len) throws IOException 
      {
          ...
          try 
          {
              ...
              while (readLength > 0) 
              {
                  ...
                  { 
                      throw new EOFException("read past EOF: " + this + " off: " + offset + " len: " + len + 
                          " pos: " + pos + " chunkLen: " + toRead + " end: " + end);
                  }
                  assert i > 0 : "FileChannel.read with non zero-length bb.remaining() must always read "
                      + "at least one byte (FileChannel is in blocking mode, see spec of ReadableByteChannel)";
                  pos += i;
                  readLength -= i;
              }
              assert readLength == 0;
